...
7. `rust/microhs-runtime/tools/wasm/browser/host.mjs` implements
   ...
   It compiles each distinct `(program, arity, body)` once, pushes args into
   `state.argbuf`, calls the cached function, and converts the result. Program
   teardown releases every registration belonging to that program.
...
- `host.mjs:mhs_js_push_str` decodes the bytes as UTF-8. The `S` tag is a text
  bridge, not a lossless arbitrary-byte bridge: malformed UTF-8 is decoded with
  the browser's replacement-character behavior.
...
- `host.mjs` injects per-runtime `UTF8ToString`, `lengthBytesUTF8`, and
  `stringToNewUTF8` helpers into compiled import bodies. It does not mutate
  `globalThis`, so multiple runtime instances cannot redirect each other's
  pointer access.
...
- Other i64 values get stable synthetic JS handles in a per-program map.
- `host.mjs:ptrFromJs` resolves only the active program's mapping, so a pointer
  from one program cannot alias storage in another. Program teardown releases
  its map even while other programs remain live.
...
7. `freeProgram` drains every remaining foreign finalizer before dropping the
   program, so live JS objects are also released when the embedder closes a
   program before its next GC.
8. Unsupported/native `host_js_obj_free` is a no-op.
...
After `m.freeProgram(handle)`, the program is removed, its host resources are
released, and its numeric handle is never reused. Export and wrapper closures
can still exist in JS, but calling one throws `MicroHs program has been freed`
before entering wasm. Monotonic handles prevent a stale closure from ever
addressing a later program.
...
Every stats counter is a JavaScript `BigInt`, because the wasm ABI transports
the values as little-endian `u64`. Convert counters explicitly before passing a
stats object to `JSON.stringify`.

### Security And CSP

JavaScript FFI bodies and `js_eval_*` inputs are executable code. The browser
host compiles them with `Function`/`eval`, so an application must treat comb
programs as trusted code with the same authority as its own JavaScript. A
Content Security Policy must allow dynamic JavaScript evaluation (normally
`script-src 'unsafe-eval'`); deployments that prohibit it cannot use this JS
FFI host. The runtime does not sandbox FFI bodies.
...
```javascript
const compiler = await createCompiler({ wasm, comb, files, packages });
const out = compiler.compile(source, { module, flags });
compiler.close();
```

`createCompiler({ wasm, comb, files, onPoll, packages })` warms one runtime
instance, preloads caller-owned absolute VFS files, and reuses the instance
across compiles.  `onPoll` is optional and is forwarded to the runtime's
...
which then reports `status: "cancelled"`.  `packages` is optional and is a list
of VFS paths to `.pkg` files, each added as `-p<path>` to every compile (both
`compile` and `toCombinators`), so pre-typechecked library modules load from the
package instead of source (see *Web Distribution* for `base.pkg`).
...
and `generated/base.pkg` as `base.pkg`.  `lib/` is copied into `include/lib/` as
a source fallback for any module not in the package; the compiler implementation
sources are already baked into the comb.  `manifest.json` maps dist include
files to VFS paths and lists `packages` (each `{ dist, vfs }`): preload every
package into the VFS at its `vfs` path and pass those paths to
`createCompiler({ packages })`.

`base.pkg` is the pre-typechecked base library, added as `-p<vfs>` to every
compile so `Prelude`/`Data.*`/… load from the package instead of being
recompiled from source.  It is the committed `generated/base.pkg` — a tracked
artifact kept in sync with `generated/mhs.comb`, regenerated by
`make generated/base.pkg` — so it replaces the old prewarm `.mhscache` / `-CR`
path: there is no dist-time cache generation and no native↔wasm cache
portability question.  The dist smoke compiles a `Prelude`+`Data.List` module
through `base.pkg` via `toCombinators`, which fails the build if the package and
comb have drifted. The tree includes the repository's Apache-2.0 `LICENSE` and
an ownership marker; the build script stages and smoke-tests a new tree before
replacing either its default output or a previously marked custom output. The
`dist/` directory is gitignored.

Package modules are inserted into the compile cache before the user module, so a
user module whose name collides with a package module (e.g. `Data.List`) is
shadowed by `base.pkg`; give user modules distinct names.
...
The browser dist no longer ships a prewarm `.mhscache`; `base.pkg` (see *Web
Distribution*) is the warm path.  `-CR` itself remains a fast, correct runtime
capability, exercised by `test-mhscache.sh`.  Reading a cache via `-CR` was
originally pathologically slow (a 471KB cache took 14+ minutes natively, and
browser runs timed out around 240s, slower than a ~90s cold compile).  The cause
was not GC: `deserialize_bfile` read the serialized graph one byte at a time and
re-parsed the whole accumulated buffer after every byte -- O(N^2).
`IO.deserialize` now parses the graph in a single streaming pass, straight from
the BFILE through the unified `Parser<ByteSource>` (`parse_graph_only` over a
`StreamSource`), so `-CR` on that cache drops to ~0.5s.
...
The public `Mhs.JavaScript` module supplies the opaque `JSVal` type:
