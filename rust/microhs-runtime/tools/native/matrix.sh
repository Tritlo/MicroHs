...
# a C compiler, and the Rust toolchain. Supports Linux and macOS, and reports
# median-of-3 wall + RSS + Rust/C ratios.
set -u
repo_root=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../../../.." && pwd)
cd "$repo_root"
OS=$(uname -s)
SELF="${MHS_SELFHOST_COMB:-/tmp/mhs-selfhost.comb}"
NORTH=0bfe5c5fc505b0b34aa6c90908dd0cdd967bfea535ed3221ad6a842133bc0884
...
if [ "$OS" = Darwin ]; then
  time_run() { /usr/bin/time -l "$@"; }
  wl()  { awk '$2 == "real" {printf "%.2f",$1; exit}' "$1"; }
  rss() { awk '/maximum resident set size/ {printf "%.0f",$1/1048576; exit}' "$1"; }
else
  time_run() { /usr/bin/time -v "$@"; }
...
if [ "$OS" = Darwin ]; then
  rm -f /tmp/cpgo.profraw /tmp/cpgo.profdata
  cc -Wall -O3 -flto -fprofile-instr-generate=/tmp/cpgo.profraw $INC $CSRC -lm -o /tmp/mhseval.gen 2>/dev/null
  /tmp/mhseval.gen +RTS $CHEAP -r"$SELF" -RTS -i -imhs -isrc -ilib MicroHs.Main -o/tmp/ctrain.comb >/dev/null 2>&1
  xcrun llvm-profdata merge -output=/tmp/cpgo.profdata /tmp/cpgo.profraw
  cc -Wall -O3 -flto -fprofile-instr-use=/tmp/cpgo.profdata $INC $CSRC -lm -o /tmp/mhseval.pgo 2>/dev/null && echo "C-PGO built (train $(ok /tmp/ctrain.comb))"
else
...
if MHS_GC_NODE_INTERVAL=$RINT bash rust/microhs-runtime/tools/native/build-selfhost-pgo.sh >/tmp/rpgo.log 2>&1; then
  echo "Rust-PGO built"
else
  cat /tmp/rpgo.log >&2
  exit 1
...
runC() { time_run "$1" +RTS $CHEAP -r"$SELF" -RTS -i -imhs -isrc -ilib MicroHs.Main -o/tmp/mx-$2$3.comb >/dev/null 2>/tmp/mx-$2$3.t
         echo "$(wl /tmp/mx-$2$3.t)"; echo "$2$3 $(wl /tmp/mx-$2$3.t)s $(rss /tmp/mx-$2$3.t)MB $(ok /tmp/mx-$2$3.comb)" >&2; }
runR() { MHS_GC_NODE_INTERVAL=$RINT time_run "$1" --input "$SELF" --mode main --warmup-iters 0 --iters 1 -- \
...
RCNP=$(med "$(rss /tmp/mx-CNP1.t)" "$(rss /tmp/mx-CNP2.t)" "$(rss /tmp/mx-CNP3.t)")
RCPG=$(med "$(rss /tmp/mx-CPG1.t)" "$(rss /tmp/mx-CPG2.t)" "$(rss /tmp/mx-CPG3.t)")
RRNP=$(med "$(rss /tmp/mx-RNP1.t)" "$(rss /tmp/mx-RNP2.t)" "$(rss /tmp/mx-RNP3.t)")
RRPG=$(med "$(rss /tmp/mx-RPG1.t)" "$(rss /tmp/mx-RPG2.t)" "$(rss /tmp/mx-RPG3.t)")
echo ""
echo "=== EQUAL-MEMORY MATRIX ($MLABEL) ==="
printf '%-14s %8s   %s\n' "runtime" "wall" "RSS"
printf '%-14s %7ss   %sMB\n' "C non-PGO"    "$CNP" "$RCNP"
printf '%-14s %7ss   %sMB\n' "Rust non-PGO" "$RNP" "$RRNP"
printf '%-14s %7ss   %sMB\n' "C PGO"        "$CPG" "$RCPG"
printf '%-14s %7ss   %sMB\n' "Rust PGO"     "$RPG" "$RRPG"
