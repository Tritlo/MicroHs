...
reduction (e.g. `B=S(KS)K`, `U=CI`, `R=CC`, `S'=B(BS)B`, `C'B=C'B`, and `Y=SPP`,
where that `P` is the SK form of `\x.f(x x)`, *not* the pairing combinator `P`).
The subtitle gives `ι` itself: `ι = λf.((f λabc.((ac)(bc))) λde.d)` = `λf. f S K`.
