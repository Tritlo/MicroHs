...
| local commits ahead after this snapshot commit | 149 |
| runtime code baseline | F2 generic strict coercion fold and rewrite/materialization counters |
...
Last fully verified state: F2 generic strict coercion fold and rewrite/materialization counter checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed after F2 generic strict coercion fold; 34 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed after F2 generic strict coercion fold |
| `cargo check -p microhs-runtime --bins --quiet` | passed after F2 generic strict coercion fold |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 generic strict coercion fold |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 generic strict coercion fold |
...
| `cargo fmt --all --check` | passed after F2 generic strict coercion fold |
| `git diff --check` | passed after F2 generic strict coercion fold |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed after F2 generic strict coercion fold |
...
| rewrite/materialization profile counters probe | passed in current checkpoint; profile output now reports generic argument materialization and spine/app rewrite extra-argument counts; self-host `--help` one-shot shows `profile_arg_materializations: 8523`, `profile_arg_materialized_nodes: 37356`, `profile_spine_rewrites: 62496`, `profile_spine_rewrite_extra_args: 251284`, `profile_app_rewrites: 226403`, and `profile_app_rewrite_extra_args: 1300478`, so app update/rebuild mechanics dominate generic argument materialization in the short compiler proxy |
...
| F2 generic strict coercion/profile probe | passed in current checkpoint; remaining scalar/pointer/array/bytes/weak/MVar `eval_*` coercions now share one `eval_whnf_value` entry that fast-checks already-WHNF values and otherwise forces through the WHNF loop, while profile rewrite counters moved to eval-loop call sites; canaries sink-match C (`arith-chain:200` Rust `35,306` vs C `108,521`, `int64-chain:200` Rust `37,806` vs C `108,862`, `float64-chain:200` Rust `46,167` vs C `126,383`, `float32-chain:200` Rust `45,533` vs C `127,072`, `bytes-chain:200` Rust `56,110` vs C `181,975`, `data-chain:300` Rust `59,828` vs C `141,740`, `zoo-chain:300` Rust `54,581` vs C `134,304`, `io-control-chain:200` Rust `53,668` vs C `142,576`, `performio-apply-chain:200` Rust `65,891` vs C `125,436`); self-host `--help` is Rust `64,076,443` ns/iter vs C `11,152,613` with matching sink `1985706` over 3 iterations; profile counts show generic materialization is smaller than app rewrite/rebuild traffic |
...
Latest tracked runtime behavior change is the current F2 generic strict coercion/profile checkpoint: remaining scalar/pointer/array/bytes/weak/MVar `eval_*` coercions share one WHNF-loop forcing helper, profile output records generic argument materialization plus spine/app rewrite extra-argument counts, and rewrite counters live at eval-loop call sites. Earlier F2 slices remain in place: the evaluator-owned WHNF spine loop, strict-result marker frames, branch/mixed-result numeric markers, unary conversion markers, direct scalar/bytes helper forcing through `reduce_whnf_from`, the strict WHNF arms `seq`, `IO.strict`, and `isInt`, and `rnf` have explicit stack/continuation paths instead of unbounded native recursion for nested chains. Latest benchmark harness change makes Rust main-mode sinks C-compatible by using the input-byte sink instead of graph-size-dependent output.
...
| `--help` main smoke | 11,152,613 ns/iter; sink `661902` per iter | 64,076,443 ns/iter; sink `661902` per iter | both run and print identical usage text; not a full compile; refreshed with `--warmup-iters 1 --iters 3 --c-mhsbench-mode main` after the F2 generic strict coercion/profile checkpoint; the proxy remains about 5.7x C on this busy machine, with unchanged reduction/node counts and new rewrite/materialization profile counters |
...
| measure | count | implication |
|---|---:|---|
| profiled iteration time | 100.653 ms | instrumentation overhead is separate from normal timing; raw time is noisy |
...
| app allocations | 347,838 | app allocation/rewrite traffic is a larger remaining signal than generic argument materialization |
| generic arg materializations | 8,523 / 37,356 nodes | generic primop/FFI argument materialization exists, but the count is small relative to rewrite extra-arg traffic |
| spine rewrites | 62,496 / 251,284 extra args | result rewrites still rebuild substantial extra spine tails |
| app rewrites | 226,403 / 1,300,478 extra args | app-style rewrites dominate the new profile counters; next F2 work should target update/rebuild mechanics here |
...
| profiled resolve calls | 5,014,674 | resolve/node-classification traffic is still much larger than reduction count; this checkpoint changes structure/counters but not resolve volume |
...
| Walk application spines directly on the evaluator stack | `evali` follows `T_AP` nodes with `PUSH(n)` until the head tag is known | The WHNF driver now owns a reusable `EvalSpine` and dispatches hot reductions from that stack; direct and generic strict coercion helpers re-enter that same loop, but generic primops/FFI still materialize head-order args before calling their helper bodies | partial |
| Rewrite the consumed redex cell and continue | `GOIND`, `GOAP`, and `GOAP2` mutate the current node/root app and jump back to `top`/`ap` | The selected F2 loop now mutates the consumed redex root directly for hot WHNF reductions and preserves root indirection for zero-arity no-spine reductions; direct strict helpers now use the same loop, while the new profile counters show full C shape mostly needs cheaper app update/rebuild for extra arguments | partial |
...
| F2 generic strict coercion/profile result | The remaining `eval_*` coercions now share one WHNF-loop forcing helper, so this closes the obvious duplicated helper shape without changing work counts. The more important result is diagnostic: generic arg materialization exists but is small in the self-host proxy, while app rewrites carry an order of magnitude more extra arguments; the next plausible F2 speed work is therefore cheaper app update/rebuild and repeated resolve/classification rather than more helper unification. |
