...
| local commits ahead after this snapshot commit | 58 |
| runtime code baseline | hot combinator app-reuse checkpoint |
...
Last fully verified state: hot combinator app-reuse checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed before hot combinator app-reuse checkpoint commit; 27 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed before hot combinator app-reuse checkpoint commit |
| `cargo check -p microhs-runtime --bins --quiet` | passed before hot combinator app-reuse checkpoint commit |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before hot combinator app-reuse checkpoint commit |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before hot combinator app-reuse checkpoint commit |
...
| `cargo fmt --all --check` | passed before hot combinator app-reuse checkpoint commit |
| `git diff --check` | passed before hot combinator app-reuse checkpoint commit |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release --bin mhs-rust-bench --quiet` | passed before hot combinator app-reuse checkpoint commit |
| `cargo build --release --bin mhs-rust --quiet` | passed before hot combinator app-reuse checkpoint commit |
...
| self-host Rust main smoke | not yet passing; no-shim run no longer falsely invokes `cpphs`; latest full compile run exceeded the old 1,200s cutoff and produced no output comb; future bounded full runs use a 600s cutoff |
...
| `arith-chain:200` | 54,337 | 106,123 | 0.51 | yes |
...
| `io-chain:200` | 59,343 | 121,254 | 0.49 | yes |
...
| `ffi-mem-chain:200` | 181,448 | 245,267 | 0.74 | yes |
...
| `bfile-read-chain:200` | 298,700 | 275,893 | 1.08 | yes |
...
| `zoo-chain:300` | 174,526 | 145,403 | 1.20 | yes |
| `data-chain:300` | 190,923 | 146,592 | 1.30 | yes |
...
| `--help` main smoke | 10,810,951 ns/iter; sink `661902` | 113,332,505 ns/iter; sink `1034891` | both run and print usage; not a full compile; refreshed with `--warmup-iters 1 --iters 3 --profile --profile-top 8`; Rust sink is runtime-internal nodes/steps, not a semantic comparison |
| self-host compiler smoke | 56,387,928,187 ns/iter; output `/tmp/mhs-selfhost-c-refresh.comb` is a 647 KiB `v8.4` comb | latest full run exceeded the old `1,200,000,000,000` ns bound at `34f017fb`; no output comb; future bounded full runs use `timeout 600s` | not at parity |
...
| measure | count | implication |
|---|---:|---|
| profiled iteration time | 145.387 ms | instrumentation overhead is separate from normal timing |
...
| nodes after run | 722,333 | hot combinator app reuse removes 167,544 transient app nodes from this proxy, though the run still grows far beyond the parsed input |
...
| self-hosting parity | C runtime can execute the compiler comb and produce a new compiler comb | Rust main-mode harness exists and lazy file IO now matches the C CPP scan smoke; latest full-compile run at `34f017fb` still exceeded the old 1,200s cutoff; future bounded attempts use 600s; inline spine work removed almost all `--help` heap-spine spills, and hot combinator app reuse cuts self-host `--help` node growth by ~19%, but full self-host parity still needs more reducer throughput work |
