...
| item | state |
|---|---|
| branch | `rust-pargc`, branched from `microhs-rust`; local work is intentionally committed regularly and may be ahead of `origin/microhs-rust` until the user asks to push. `split-runtime-modules` was fast-forward merged and remains as the source-organization side branch |
| committed checkpoint | `self-hosting-binary-match`; latest accepted current-source performance checkpoint is `67d90850` (`Merge tiny eval modules`), which is neutral-to-slightly-positive versus `01c0fe74` in the paired full gate below. Prior runtime performance checkpoint is the rev-5 profile cfg-gate row, following the packed-cell/sweep/profile-surface cleanup series. Earlier source-organization checkpoint is `8933a6d1` (`Split live reducer modules`), following `76ff9d3e` (`Use real runtime modules`), `b8284e1e` (`Reorganize Rust runtime tree`), and `a28d0290` (`Split runtime implementation into include files`); latest WASI self-host harness checkpoint is `c5f8ea1f` (`Add WASI self-host harnesses`); latest wasm browser-host checkpoint is `61eca83b` (`Add browser host filesystem for wasm self-host`); latest wasm comparison harness checkpoint is `b49dc2ef` (`Add wasm32 runtime comparison harness`); older runtime checkpoints include `d5efc37f` (`Trust hot reducer cell reads`), `c969e993` (`Trust app-fun descent in reducer hot path`), and `3c04e5c1` (`Trust WHNF resolver in reducer hot path`); latest self-host harness checkpoint is `15f76b3a` (`Add self-host neutrality benchmark harness`) |
| active working tree | runtime now uses the accepted packed 8-byte `Cell` arena unconditionally; the obsolete `packed-cell` feature and 16-byte fallback were removed after a neutral A/B against `11334945`. Current source organization is real Rust modules, not `include!`: `runtime.rs` declares the public facade and internal module tree; `runtime/host.rs`, `runtime/program.rs`, `runtime/program/bfile.rs`, `runtime/program/eval.rs`, `runtime/program/runtime_dispatch.rs`, `runtime/program/values.rs`, and `runtime/eval_state.rs` are `mod` roots with scoped imports. `eval_state` is split into focused cache/spine/head/profile-frame/frame/stack modules instead of a 1k-line catch-all root; `host/filesystem` is split into browser-host, env, path, permission, directory, native-file, and fd/socket modules instead of a target-heavy 1k-line catch-all file; `program/values` is split into force/bytes/memory/syscall/bigint helper modules instead of one mixed value-and-host helper file. The live fallback reducer is no longer named legacy: `eval/reduce.rs` owns the general delegated loop plus eval-spine helpers, `eval/stack.rs`/`eval/frames.rs` own the active WHNF stack driver and strict stack frames, and `eval/dispatch.rs` owns eval-time primitive helpers plus fallback runtime-primitive rewriting. The old unreachable persistent-spine/non-stack WHNF driver has been deleted. Release builds use `panic = "abort"`, `lto = "thin"`, and `codegen-units = 1`. Native helpers live under `rust/microhs-runtime/tools/native/`; browser JS-FFI wasm harnesses live under `rust/microhs-runtime/tools/wasm/browser/`; the WASI self-host harness lives under `rust/microhs-runtime/tools/wasm/wasi/`; the native bench scenario catalog is split under `src/bin/mhs_rust_bench/scenarios/` by pure, IO, FFI, and BFILE inputs. Runtime profiling is cfg-gated behind the `profile` cargo feature, so default builds do not carry `Program.profile` or nonzero profile-head frame payloads; the old detailed `eval-phase-profile` layer has been deleted. Current runtime also has the GC-rooted read-only memory BFILE view, checked LZMA decoding, weak-pointer key/value GC semantics, and the self-host/PGO harnesses. The rejected `moving-gc` scaffold has been deleted; active GC is the default mark/sweep path only |
...
| gate | status |
|---|---|
| Rust fmt/test/release bench build | latest current-source gates after the benchmark scenario module split: all 50 original size-3 scenario outputs matched the pre-split length/FNV snapshot byte-for-byte; `cargo fmt --check`, `cargo check --all-targets`, `cargo check --all-targets --features profile,gc-phase-profile`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, and `git diff --check` passed. No self-host rerun: this is benchmark-input generation only. Runtime/perf gates on a shared or noisy server should pair a same-window C control with the Rust measurement. Prior runtime test module split also passed source grep for stale `legacy`/`persistent`/`moving-gc`/`eval-phase-profile`/`WRAPBUG`/`include!` residue. Prior FFI pointer load/store helper cleanup also passed `cargo check --features profile`, `cargo check --features gc-phase-profile`, release `mhs-rust-bench` build, `cargo check --target wasm32-unknown-unknown`, `cargo check --target wasm32-wasip1 --bin mhs-rust`, 100M/32M bounded self-host sanity, and paired C/Rust 128M full self-host noise-control sample with SHA/cmp. Prior `program/values`, `host/filesystem`, `eval_state`, and strict-action/test-wrapper/let-return simplification gates passed the same native check/test/release/sanity set. Prior architecture-doc gates also passed `cargo doc --no-deps`. Prior profile-counter rename gates also passed `cargo check --features profile`, profile-enabled release `mhs-rust-bench` build, 1M profile smoke, default release `mhs-rust-bench` rebuild, and broad source grep for stale live `persistent` profile names. Prior broad gates after deleting the dead persistent/non-stack reducer layer also passed `cargo check --features gc-phase-profile`. Prior broad gates after deleting the rejected `moving-gc` scaffold and stale `eval-phase-profile` layer also passed `cargo check --features profile,gc-phase-profile`, `cargo check --all-targets --features profile`, release `wasm32-unknown-unknown` build, and release `wasm32-wasip1 --bin mhs-rust` build. Historical feature-gated moving-GC/eval-phase rows below remain as rejected/retired experiment records only |
| runtime architecture docs | latest docs update extends `rust/microhs-runtime/ARCHITECTURE.md` with the current module tree, WHNF stack-reducer flow, GC-root invariants, target-boundary split, and source-organization verification contract; `cargo doc --no-deps` passed. Docs-only checkpoint; no self-host rerun needed |
| stack rewrite profiling verification | detailed stack phase/shape profiling was retired with the `eval-phase-profile` deletion. Basic `--profile` still works and now prints aggregate head/reduction/app/stack/fallback counters only; the latest profile-enabled 1M smoke completed with `profile_steps=1,001,388`, sink `661902`, `profile_strict_forces=35,793`, `profile_fallback_entries=90`, and no retired phase/shape sections |
...
| parallel-gc sweep phase-1 | active experiment: added feature-gated `parallel-gc` with deterministic parallel sweep only; default builds keep the sequential collector. `MHS_GC_THREADS=1` uses the sequential sweep, while higher counts partition the arena on 64-cell boundaries, build local free-list segments, and stitch them back in stable descending-index order. Because current `cold_nodes` stores separate cold-table indices rather than `NodeId` indices, cold payload slots are cleared after worker join instead of from worker-owned arena ranges. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo check --all-targets --features parallel-gc,gc-phase-profile`, `cargo test --lib` 41/41 default, `cargo test --lib --features parallel-gc` 41/41, `cargo test --lib --features parallel-gc,gc-phase-profile` 41/41, and `git diff --check`. 100M/32M bounded self-host smoke with `MHS_GC_THREADS=4` and candidate `parallel-gc,gc-phase-profile`: base `1964.662ms`, 3 GCs, `233.010ms` GC pause, high-water `33,952,444`, sink `661902`; candidate `1651.951ms` (`-15.92%`), same `100,807,543` steps, 3 GCs, high-water `33,952,444`, sink `661902`, `135.137ms` GC pause with `72.408ms` mark and `44.411ms` sweep. `cmp_result=missing-output` is expected for step-limited runs. Phase 2 parallel mark is still open; current mark-time canonicalization can rewrite child/target cells while visiting a parent, so it needs a race-free design rather than a direct atomic worklist drop-in |
| parallel-gc 128M full self-host | 3x full self-host A/B at `MHS_GC_NODE_INTERVAL=134217728`, `MHS_GC_THREADS=16`, candidate features `parallel-gc,gc-phase-profile`, `MHS_STEP_LIMIT=none`: all six outputs matched SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp_result=0`, same `3,659,609,666` steps, 31 GCs, high-water `138,051,734`, sink `661902`. Base runs were `59.487s`, `58.630s`, `58.704s` (avg `58.941s`) with avg GC pause `10.339s`; parallel-sweep candidate runs were `56.079s`, `55.608s`, `55.701s` (avg `55.796s`, `-5.34%`) with avg GC pause `6.803s`. Candidate phase averages: mark `4.634s`, sweep `1.967s`. Reading: sweep parallelism buys a real full-self-host pause reduction and wall win even with `gc-phase-profile` enabled, but mark is still sequential and now dominates the remaining pause |
| parallel-gc 80Mi full self-host | 3x full self-host A/B at `MHS_GC_NODE_INTERVAL=83886080`, `MHS_GC_THREADS=16`, candidate features `parallel-gc,gc-phase-profile`, `MHS_STEP_LIMIT=none`: all six outputs matched SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp_result=0`, same `3,659,609,761` steps, 50 GCs, high-water `88,211,550`, sink `661902`. Base runs were `57.941s`, `57.777s`, `58.093s` (avg `57.937s`) with avg GC pause `12.347s`; parallel-sweep candidate runs were `54.161s`, `54.103s`, `54.404s` (avg `54.222s`, `-6.41%`) with avg GC pause `8.965s`. Candidate phase averages: mark `6.886s`, sweep `1.945s`. Reading: the fair-memory regime benefits more than 128M because there are more sweeps; parallel sweep cuts pause materially, but sequential mark is now most of the remaining collector cost |
| parallel-gc persistent sweep pool | accepted as the next sweep slice: replaced per-collection scoped thread creation with feature-gated process-lifetime sweep pools keyed by worker count. Workers stay detached and idle between stop-the-world collections; each GC submits disjoint arena/mark ranges, waits for all workers, then stitches local free-list segments in fixed range order, so the deterministic free-list invariant is unchanged. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo check --all-targets --features parallel-gc,gc-phase-profile`, `cargo test --lib`, `cargo test --lib --features parallel-gc,gc-phase-profile`, `cargo check --target wasm32-unknown-unknown --lib --features parallel-gc`, `cargo check --target wasm32-wasip1 --bin mhs-rust --features parallel-gc`, and `git diff --check`. 3x 80Mi full self-host A/B at `MHS_GC_NODE_INTERVAL=83886080`, `MHS_GC_THREADS=16`, candidate `parallel-gc,gc-phase-profile`, `MHS_STEP_LIMIT=none`: all six outputs matched SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp_result=0`, same `3,659,609,970` steps, 50 GCs, high-water `88,211,599`, sink `661902`; base avg `58.116s`, GC pause `12.346s`; candidate avg `54.898s` (`-5.54%`), GC pause `9.041s`, mark `6.915s`, sweep `1.991s`. 3x 128M full self-host A/B at `MHS_GC_NODE_INTERVAL=134217728`: all six outputs matched the same SHA, `cmp_result=0`, same `3,659,609,856` steps, 31 GCs, high-water `138,051,754`, sink `661902`; base avg `56.332s`, GC pause `9.713s`; candidate avg `52.493s` (`-6.82%`), GC pause `6.499s`, mark `4.421s`, sweep `1.910s`. Reading: this satisfies the persistent-worker guardrail for the accepted sweep path, but mark is still sequential and still dominates the remaining pause |
| parallel-gc thread-count matrix | acceptance matrix for the accepted sweep-only collector with persistent workers. Built `mhs-rust-bench --features parallel-gc,gc-phase-profile` once, then ran full self-host directly with a fixed output path for every sample so guest compiler arguments were identical across thread counts. Each point is 3 runs averaged. 128M (`MHS_GC_NODE_INTERVAL=134217728`, 31 GCs, steps `3,659,610,217`, high-water `138,051,792`, sink `661902`): threads 1 avg `55.339s`, GC `9.585s`, mark `4.422s`, sweep `4.983s`; 2 avg `52.945s`, GC `7.092s`, mark `4.436s`, sweep `2.489s`; 4 avg `52.458s`, GC `6.608s`, mark `4.432s`, sweep `1.993s`; 8 avg `52.900s`, GC `6.639s`, mark `4.494s`, sweep `1.960s`; 16 avg `52.818s`, GC `6.619s`, mark `4.528s`, sweep `1.926s`. 80Mi (`MHS_GC_NODE_INTERVAL=83886080`, 50 GCs, steps `3,659,610,217`, high-water `88,211,614`, sink `661902`): threads 1 avg `57.164s`, GC `12.270s`, mark `6.897s`, sweep `5.229s`; 2 avg `54.637s`, GC `9.673s`, mark `6.927s`, sweep `2.605s`; 4 avg `55.519s`, GC `9.211s`, mark `6.937s`, sweep `2.137s`; 8 avg `54.220s`, GC `9.068s`, mark `6.868s`, sweep `2.063s`; 16 avg `54.238s`, GC `8.987s`, mark `6.878s`, sweep `1.972s`. All 30 corrected outputs matched SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`; steps/high-water/sink were identical across thread counts within each heap regime. Reading: sweep scaling is real from 1->2 and mostly saturates by 4 threads; the remaining pause is mark-dominated, so this matrix reinforces parallel mark as the next required implementation step |
| rejected mark prefetch probes | rejected and reverted before full gate: `NOTES.md` suggested software prefetch as the cheapest remaining mark-phase lever, so three x86/x86_64-only `parallel-gc` prefetch variants were compared against the committed no-prefetch sweep-pool binary on the same fixed-argument 100M self-host slice (`MHS_GC_THREADS=8`, 3 alternating repeats each). Step-limited outputs have no final file, but all pairs had identical steps, GCs, high-water, and sink. Broad frontier + App-child + array + IND-chain prefetch regressed mark `73.365ms -> 81.467ms` and GC `145.639ms -> 150.953ms`; App-child-only prefetch regressed mark `72.147ms -> 73.823ms` with neutral GC `141.126ms -> 140.627ms`; frontier-only prefetch regressed mark `72.726ms -> 78.933ms` and GC `144.163ms -> 151.284ms`. Reading: manual prefetch adds instruction overhead and/or bad cache pressure on this mark loop; do not rerun prefetch without a more specific hardware-counter rationale |
| rejected parallel mark probes | rejected and reverted before commit: three feature-gated std-only parallel mark sketches were byte-identical where run but not competitive with the accepted sweep-only branch. Read-only atomic mark plus sequential normalization regressed the 100M self-host smoke by `+9.81%` with GC pause `392.602ms` vs base `217.770ms`; recorded-rewrite marking with mutex queues regressed 100M by `+9.87%` with GC pause `324.267ms`; a local-stack sharing variant improved its same-window 100M smoke (`1792.126ms` -> `1730.064ms`, `-3.46%`) but still had candidate GC pause `221.424ms` with mark `146.298ms`, far worse than sweep-only mark cost. One 80Mi full self-host scout of that local-stack variant was byte-identical (`cmp_result=0`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, same `3,659,609,932` steps, 50 GCs, high-water `88,211,605`, sink `661902`) at base `64.503s` vs candidate `59.724s`, GC pause `13.393s` -> `9.064s`, candidate mark `6.875s`, sweep `2.043s`; that was not promoted to 3x because it did not beat the already accepted sweep-only 80Mi 3-run candidate avg `54.222s` and did not reduce mark time (`6.886s` sweep-only). Reading: the safe drop-in mark variants add more coordination than they remove; parallel mark needs a different representation/work-stealing design before it deserves full 3x gates |
| wasm library | release `cargo build --target wasm32-unknown-unknown` passed on current `15f76b3a`; Node `host.mjs` smoke rendered `2147483648` from `v8.4\n0\n#2147483648 }\n`, so Rust's wasm path does not inherit the C/eval.c ILP32 `intptr_t` wrap caused by C `value_t` following target pointer width; current `wasm32-wasip1` release bench builds and the committed pure-JS WASI shim self-host harness completes byte-identically in `135.941s` with 124 GCs and `38.420s` GC pause. Browser-path comparison now has a Node-checkable harness: `rust/microhs-runtime/tools/wasm/browser/build-browser-bench.sh` builds Rust `wasm32-unknown-unknown` plus an Emscripten ES module from `src/runtime/mhsbench.c`, `browser-compare.mjs` compares parse+WHNF+serialize with matching sinks, `browser-selfhost.mjs` tests Emscripten/browser self-hosting, and `wasi-selfhost.mjs` tests the Rust `wasm32-wasip1` CLI path. Latest 1000-iter/100-warmup Node evaluator pass matched all sinks and showed Rust faster on all eight first rows. Self-host status: Rust browser wasm completed byte-identically from the JS-backed host env/filesystem in 180.627s; fresh C/Emscripten wasm completes from MEMFS in 61.464s but output differs from native by the known eval.c wasm32 wrap/float issue; Rust `wasm32-wasip1` completes byte-identically in 135.941s |
| EVALLESSONS correctness batch | current `cargo test --manifest-path rust/microhs-runtime/Cargo.toml --lib` passes 41/41; covers F1 string escapes, F3 catchable arithmetic RTS exceptions, F4 uncaught RTS/showExn formatting and `ExitSuccess` handling, F6 shared/cyclic serializer labels plus `IO.deserialize` memory-BFILE/cycle smokes, F7 unknown prim failures, F10 iterative deep serializer plus BFILE `IO.print`/`IO.serialize` smokes, F11 mpz wire format, F12 catch handler masking/restoration smoke, F15 `putchar`/`lz77c`, F16 faithful modified UTF-8 decoding, F17 immutable bytestring views for `bssubstr`/`tailUTF8`, F18 ignored-IO shortcut depth cap, F19 compression fixture/round-trip coverage, F21 `mpz_get_d` decimal rounding, weak-pointer GC key/value reachability, and F14 focused null-finalizer GC plus direct `^&closeb` address-FFI smokes. The old moving-GC remapper/evacuation tests were removed with the rejected scaffold |
...
| latest program-values module split gate | `program/values.rs` is now a thin real-module root instead of a 782-line mixed value/host helper file; moved WHNF forcing and scalar/cold-node projection, byte/mutable-byte operations, C allocation/errno/host-int helpers, syscall/socket marshaling helpers, and bigint foreign-pointer helpers into focused submodules. Verification passed the latest Rust fmt/check/test/release/wasm/WASI gates above, 100M/32M bounded self-host sanity (`1.721s`, `58.59M` steps/s, `100,811,779` steps, 3 GCs, `193.055ms` GC, high-water `33,939,357`, sink `661902`), and a 3x 128M full gate at median `53.878s`, avg `54.072s`, byte-identical SHA/cmp. Organization-only cleanup, not a performance claim |

| latest host-filesystem module split gate | `host/filesystem.rs` is now a thin real-module root instead of a 988-line target-heavy catch-all; moved browser host-result glue, environment operations, path/process/tmpname operations, permissions, directory entries, native BFILE open/mode parsing, and fd/socket wrappers into focused submodules. Verification passed the latest Rust fmt/check/test/release/wasm/WASI gates above, 100M/32M bounded self-host sanity (`1.944s`, `51.86M` steps/s, `100,811,779` steps, 3 GCs, `218.682ms` GC, high-water `33,939,367`, sink `661902`), and a 3x 128M full gate at median `53.647s`, avg `53.772s`, byte-identical SHA/cmp. Organization-only cleanup, not a performance claim |

| latest eval-state module split gate | `eval_state.rs` is now a thin real-module root instead of a 1,036-line catch-all; moved cached roots, transient spines, runtime-primitive head/action lookup, profile frame payloads, strict frame payloads, and explicit stack operations into focused submodules (`cache.rs`, `spine.rs`, `head.rs`, `profile_head.rs`, `frames.rs`, `stack.rs`). Verification passed the latest Rust fmt/check/test/release/wasm/WASI gates above, 100M/32M bounded self-host sanity (`1.710s`, `58.95M` steps/s, `100,811,779` steps, 3 GCs, `190.554ms` GC, high-water `33,939,357`, sink `661902`), and a 3x 128M full gate at median `53.544s`, avg `53.722s`, byte-identical SHA/cmp. Organization-only cleanup, not a performance claim |
...
| GC/F5 | safe-point mark/sweep plus S2 allocation-pressure trigger, intrusive free list, mark-time indirection compression, reusable mark-work stack, parse-label non-root treatment, optional `gc-phase-profile` mark/sweep timings, C-style sweep-time ForeignPtr finalizers, and eval.c-style weak key/value clearing are implemented. The failed `moving-gc`/minor-collector scaffold is deleted; scheduler-backed weak finalizer spawning and any future selective GCRED/mark-slot semantics remain later work |
...
but its output is not a correctness oracle because of the known eval.c wasm32 divergence.
...
## 2026-07-05 Rev-7 PGO Floor Check

| item | result |
|---|---|
| setup | measurement-only PGO floor check from `NOTES.md` rev 7. Ran `rust/microhs-runtime/tools/native/build-selfhost-pgo.sh` with `MHS_PGO_DIR=/tmp/mhs-rust-pgo-rev7-floor`, `MHS_GC_NODE_INTERVAL=134217728`, and `MHS_PGO_TRAIN_TIMEOUT=900s`; the PGO-use binary was `/tmp/mhs-rust-pgo-rev7-floor/use/release/mhs-rust-bench`. This does not change source or release defaults and must not be shipped as the answer |
| training run | instrumented 128M full self-host completed byte-shaped in `81.229s`, `3,659,074,926` steps, `31` GCs, `10.461s` GC pause, high-water `138,068,210`, sink `661902`; produced `/tmp/mhs-rust-pgo-rev7-floor/merged.profdata` |
| PGO 128M default 3x | byte-identical outputs (`cmp=0`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, size `661,784`) at `45.777s`, `45.022s`, `45.473s`; median `45.473s`, avg `45.424s`. All samples used `3,659,074,793` steps, `31` GCs, high-water `138,068,138`, sink `661902`, and `cell_size_bytes=8`. GC pauses were `9.407s`, `9.325s`, `9.271s`; median `9.325s`, avg `9.335s` |
| PGO 80Mi fair 3x | byte-identical outputs (`cmp=0`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, size `661,784`) at `49.421s`, `51.583s`, `47.014s`; median `49.421s`, avg `49.339s`. All samples used `3,659,074,736` steps, `49` GCs, high-water `88,151,680`, sink `661902`, and `cell_size_bytes=8`. GC pauses were `12.381s`, `12.848s`, `11.657s`; median `12.381s`, avg `12.295s` |
| delta | against current source medians, Rust PGO 128M is `-15.2%` versus the `53.636s` best current-source gate and `-17.8%` versus the latest `55.306s` refresh. Rust PGO 80Mi is `-11.6%` versus the current fair-memory `55.926s` median. Relative to the older non-PGO C oracle `47.85s`, Rust PGO 128M looked faster (`0.95x C`) at the larger speed heap, while Rust PGO 80Mi was `1.03x C` at roughly C-sized memory; A.2 below is the fair comparison and supersedes that headline for floor analysis |
| reading | this quantifies the Rust codegen/layout floor rather than a source win. PGO buys a large Rust speedup, but the fair C-PGO/matched-heap matrix below shows the remaining gap is not erased once the C side gets the same compiler and heap treatment. Per directive, close performance probing here and pivot to review/simplification; do not ship PGO |

## 2026-07-05 Rev-7 A.2 C PGO Fair Matrix

| item | result |
|---|---|
| setup | measurement-only A.2 follow-up from `NOTES.md` rev 7. Built temporary C binaries under `/tmp/mhs-c-pgo` with GCC `13.3.0`: O3 used the existing recipe shape (`cc -Wall -O3 -Isrc/runtime -Isrc/runtime/unix src/runtime/main.c src/runtime/eval.c generated/mhs.c -lm`); PGO used `-fprofile-generate=/tmp/mhs-c-pgo/prof`, one self-host training run, then `-fprofile-use=/tmp/mhs-c-pgo/prof -fprofile-correction` rebuilt at the same output path so GCC found the generated `.gcda` files. All measured runs used `+RTS -v -RTS` or `+RTS -v -H134217728 -RTS`, `timeout 900s`, and `-i -imhs -isrc -ilib MicroHs.Main -o/tmp/mhs-c-pgo/out/...comb` |
| output | all 12 measured outputs were byte-identical to the reference: SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, size `661,784` bytes. The training output matched the same SHA. C's `max cells used` field is the live-mark high-water printed by eval.c, not Rust's arena high-water |

| C build | C default heap, `50,000,000` cells | C matched heap, `134,217,728` cells |
|---|---:|---:|
| `-O3` | `46.90s`, `46.40s`, `46.23s`; median `46.40s`, avg `46.51s`; `89` GCs; median GC `11.42s`; median mark/scan `11.14s`/`0.28s`; max cells used `5,953,479`; reductions `3,651,955,968`; allocated `4,131,061,883` cells | `40.45s`, `40.30s`, `40.26s`; median `40.30s`, avg `40.34s`; `34` GCs; median GC `4.45s`; median mark/scan `4.32s`/`0.14s`; max cells used `3,919,967`; reductions `3,655,806,987`; allocated `4,131,061,958` cells |
| PGO | `45.73s`, `45.59s`, `45.42s`; median `45.59s`, avg `45.58s`; `89` GCs; median GC `11.98s`; median mark/scan `11.69s`/`0.30s`; max cells used `5,953,484`; reductions `3,651,955,936`; allocated `4,131,061,930` cells | `39.28s`, `38.61s`, `38.62s`; median `38.62s`, avg `38.84s`; `34` GCs; median GC `4.62s`; median mark/scan `4.48s`/`0.14s`; max cells used `3,919,948`; reductions `3,655,806,887`; allocated `4,131,062,005` cells |

| reading | result |
|---|---|
| C heap effect | the matched `134217728`-cell heap is a large C win by itself: O3 default `46.40s` -> O3 matched `40.30s` (`-13.1%` median), mostly by cutting GC from `89` collections / `11.42s` to `34` collections / `4.45s` |
| C PGO effect | GCC PGO is modest at the default heap (`46.40s` -> `45.59s`, `-1.7%`) and larger at the matched heap (`40.30s` -> `38.62s`, `-4.2%`) |
| decisive peer | the fair floor peer is Rust-PGO@128M median `45.473s` versus C-PGO@`134217728` median `38.62s`: Rust remains `1.18x` C in that equalized PGO/heap comparison. The shipped headline remains default Rust release versus C O3 at normal configs, not PGO builds |

## 2026-07-05 Rev-7 A.3 C 80M Matched-Memory PGO Column

| item | result |
|---|---|
| setup | measurement-only A.3 follow-up from `NOTES.md` rev 7. Built temporary C binaries under `/tmp/mhs-c-pgo-h80000000` with GCC `13.3.0` and `-DHEAP_CELLS=80000000`. `-O3` used `cc -Wall -O3`; PGO used `-O3 -flto -fprofile-generate=/tmp/mhs-c-pgo-h80000000/prof`, one full self-host training run, then `-O3 -flto -fprofile-use=/tmp/mhs-c-pgo-h80000000/prof` rebuilt at the same output path so GCC found the generated profile data. All measured runs used `+RTS -v -RTS`, `timeout 900s`, and `-i -imhs -isrc -ilib MicroHs.Main -o/tmp/mhs-c-pgo-h80000000/out/...comb` |
| output | all 6 measured outputs were byte-identical to `/tmp/mhs-selfhost.comb`: SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, size `661,784` bytes, `cmp=0` for every sample. The PGO training output also completed byte-shaped in `47.95s` with `55` GCs and `7.75s` GC |

| C build | `80,000,000`-cell heap samples |
|---|---:|
| `-O3` | `43.87s`, `43.17s`, `43.34s`; median `43.34s`, avg `43.46s`; `55` GCs; GC pauses `7.45s`, `7.38s`, `7.44s` (median `7.44s`, avg `7.423s`); median mark/scan `7.23s`/`0.20s`; max cells used `4,235,014`; reductions `3,653,643,721`; allocated `4,131,064,113` cells |
| PGO | `41.52s`, `41.16s`, `40.78s`; median `41.16s`, avg `41.15s`; `55` GCs; GC pauses `7.52s`, `7.50s`, `7.47s` (median `7.50s`, avg `7.497s`); median mark/scan `7.31s`/`0.20s`; max cells used `4,235,010`; reductions `3,653,643,723`; allocated `4,131,064,160` cells |

| comparison | result |
|---|---|
| C heap effect vs default | A.3 `80M` sits between the A.2 default and 128M columns: O3 default `46.40s` -> O3 80M `43.34s` (`-6.6%`) -> O3 128M `40.30s`; PGO default `45.59s` -> PGO 80M `41.16s` (`-9.7%`) -> PGO 128M `38.62s` |
| PGO effect at 80M | GCC PGO buys `43.34s` -> `41.16s` (`-5.0%`) at the 80M heap; GC time is essentially unchanged, so the win is codegen/layout, not collection frequency |
| fair 80Mi peer | Rust PGO@80Mi median `49.421s` versus C PGO@80M median `41.16s`: Rust remains `1.20x` C in the fairer-memory PGO comparison. Default Rust@80Mi median `55.926s` versus C O3@80M median `43.34s`: Rust is `1.29x` C without PGO |
...
| stale app-allocation timing trim | accepted as profile-surface cleanup: removed the eval-phase-only app-allocation site nanosecond map and the profiling-overhead timer for app-allocation bookkeeping. The useful allocation diagnostics remain: total app allocations, aggregate app-allocation time, app allocation site counts, and resolved app-allocation site shapes. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --all-targets`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, release `mhs-rust-bench --features eval-phase-profile` build, and a 1M eval-profile smoke (`profile_total_ms=1210.392`, `1,001,388` steps, sink `661902`) that still prints app-allocation site counts and resolved shapes but no `profile_stack_app_alloc_site_ms` or `profile_profile_app_alloc_bookkeeping_ms`. Final 100M/32M sanity was `1771.262ms`, `56.92M` steps/s, `100,811,779` steps, `3` GCs, `197.954ms` GC pause, high-water `33,939,304`, sink `661902`, `cell_size_bytes=8`. No full self-host gate: this trims feature-only profile output/hooks and leaves default reduction behavior unchanged |
| stale per-head eval timing trim | accepted as profile-surface cleanup: removed the eval-phase-only per-head nanosecond maps for stack eval-step, arg-read, rewrite, app-update, force-frame, and inner-descent timing, plus the profile-overhead counter for maintaining those maps. Aggregate phase timers and the useful count distributions remain: head arities/classes, continuation transitions, app allocation site counts, and resolved site shapes. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --all-targets`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, release `mhs-rust-bench --features eval-phase-profile` build, a 1M eval-profile smoke (`profile_total_ms=785.398`, `1,001,388` steps, sink `661902`) that no longer prints the per-head timing sections, and `git diff --check`. Final 100M/32M sanity was `1734.803ms`, `58.11M` steps/s, `100,811,779` steps, `3` GCs, `196.082ms` GC pause, high-water `33,939,314`, sink `661902`, `cell_size_bytes=8`. Because this cleaned guarded calls inside `stack.rs`, the 128M full gate was refreshed 3x: byte-identical at `56.419s`, `56.542s`, `58.838s` (median `56.542s`, avg `57.266s`), `31` GCs, median GC pause `8.747s`, high-water `138,068,154`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp=0` |
| stale profile-overhead timing trim | accepted as profile-surface cleanup: removed the eval-phase-only timers that measured time spent inside `profile_step` and `profile_reduction`. Head attempt/reduction counts, arity classes, continuation transitions, aggregate phase timers, app allocation site counts, and resolved site shapes remain. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --all-targets`, `cargo test --lib` 43/43, release `mhs-rust-bench --features eval-phase-profile` build, a 1M eval-profile smoke (`profile_total_ms=706.928`, `1,001,388` steps, sink `661902`) with no `profile_profile_step_ms` or `profile_profile_reduction_ms` rows, default release `mhs-rust-bench` rebuild, and 100M/32M sanity (`1777.801ms`, `56.71M` steps/s, `100,811,779` steps, `3` GCs, `196.627ms` GC pause, high-water `33,939,318`, sink `661902`, `cell_size_bytes=8`). No full self-host gate: this removes feature-only profiling-overhead counters and does not change default reduction semantics |
| stale stack-rethread profiling trim | accepted as profile-surface cleanup: removed the always-profiled `stack_rethreads`/`stack_rethread_apps` counters, output rows, and the guarded frame-path hook. Current useful stack counters remain: rewrite/app-update counts, descent pushes, arg reads/batches, frame pushes, arity classes, and continuation transitions. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --all-targets`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, 100M/32M sanity (`1765.389ms`, `57.10M` steps/s, `100,811,779` steps, `3` GCs, `192.047ms` GC pause, high-water `33,939,314`, sink `661902`, `cell_size_bytes=8`), release `mhs-rust-bench --features eval-phase-profile` build, a 1M eval-profile smoke (`profile_total_ms=718.081`, `1,001,388` steps, sink `661902`) with no `profile_stack_rethreads` rows, and a final default release bench rebuild. No full self-host gate: this removes a stale profile counter and one `self.profile.is_some()` branch in the cold frame rethread path |
...
| node trace helper inline | accepted as cold diagnostic cleanup: after invalid-byte tracing was removed, `runtime_dispatch/trace.rs` only held the single `node_trace_summary` formatter used by unsupported foreign-finalizer errors. Inlined that helper into `handles.rs` and removed the orphan dispatch trace module; the real error payload remains unchanged. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc`, `cargo check --all-targets`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, and `git diff --check`. Final 100M/32M sanity was `1725.772ms`, `58.42M` steps/s, `100,811,779` steps, `3` GCs, `197.949ms` GC pause, high-water `33,939,310`, sink `661902`, `cell_size_bytes=8`. No full self-host gate: this moves one cold formatter and deletes an orphan module, not evaluator or allocator behavior |
| dispatch inner-wrapper trim | accepted as local source cleanup: after the temporary dispatch error tracers were removed, `bytes_op`, `bytes_unop`, `ffi_call`, `foreign_ptr_op`, and `foreign_ptr_unop` only delegated to same-file `*_inner` functions. Removed those pass-through wrappers and kept the existing caller-facing method names. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc`, `cargo check --all-targets`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, and `git diff --check`. Final 100M/32M sanity was `1736.485ms`, `58.06M` steps/s, `100,811,779` steps, `3` GCs, `195.461ms` GC pause, high-water `33,939,318`, sink `661902`, `cell_size_bytes=8`. No full self-host gate: this removes pass-through source wrappers but leaves dispatch order and evaluated operations unchanged |
| JS FFI host split | accepted as wasm/host structure cleanup: split `host/js_ffi.rs` into shared tag/int helpers plus target-specific `browser` and `unsupported` modules, removing repeated per-function cfg branches while keeping the same `host_js_*` surface and C ABI externs. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --all-targets`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, and `git diff --check`. Final 100M/32M sanity was `1757.314ms`, `57.37M` steps/s, `100,811,779` steps, `3` GCs, `197.787ms` GC pause, high-water `33,939,300`, sink `661902`, `cell_size_bytes=8`. No full self-host gate: native evaluator behavior is unchanged and the important gate for this cleanup is browser/WASI wasm compilation |
| JS FFI type locality | accepted as wasm/host structure cleanup: moved `JsArg` and browser-only `JsValue` out of core heap types and into `host/js_ffi.rs`, with `runtime::JsValue` still re-exported for the browser wasm facade. The non-browser dead-code allowance now lives on the JS FFI argument type instead of in `core_types.rs`. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --all-targets`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, and `git diff --check`. Final 100M/32M sanity was `1720.192ms`, `58.60M` steps/s, `100,811,779` steps, `3` GCs, `188.066ms` GC pause, high-water `33,939,302`, sink `661902`, `cell_size_bytes=8`. No full self-host gate: this is type/module locality cleanup and native evaluator behavior is unchanged |
| stale WASI host trace trim | accepted as disabled-diagnostic cleanup: removed the old `MHS_WASI_TRACE` host-call tracing helpers plus their filesystem and native-BFILE call sites. The trace was useful during the earlier WASI ENOSYS/memory-growth investigation, but the current WASI path self-hosts and normal execution should not carry the extra target-gated debug surface. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc`, `cargo check --all-targets`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, and release `wasm32-wasip1 --bin mhs-rust` build. Final 100M/32M sanity was `1796.472ms`, `56.12M` steps/s, `100,811,779` steps, `3` GCs, `193.080ms` GC pause, high-water `33,939,306`, sink `661902`, `cell_size_bytes=8`. No full self-host gate: this removes WASI-only diagnostic printing, not evaluator/allocator semantics |
| tracked old makefile removal | accepted as repository-surface cleanup: deleted `Makefile.old`, the stale pre-current makefile snapshot last touched by the early `Start of new Makefile` / `Make it compile` commits. `git grep` found no references to `Makefile.old`; the live `Makefile` still owns the documented `oldinstall`/`oldminstall` targets, so install behavior is unchanged. Verification passed `git grep -n "Makefile.old\\|oldinstall"` before deletion, confirming only live `oldinstall` docs/targets remain. No benchmark gate: this removes an unused legacy build file, not runtime code |
| cold clippy noise trim | accepted as performance-neutral warning cleanup: removed a needless slice borrow in memory BFILE writes, replaced two impossible `socklen_t -> u64` fallible conversions with `u64::from`, and switched one test byte-slice assertion to a byte string. This deliberately skipped clippy's broad collapsible-if suggestions in evaluator/GC hot code. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --all-targets`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc`, `cargo test --lib` 43/43, `cargo clippy --all-targets -- -A clippy::needless_return` with the targeted warnings gone, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, and release `wasm32-wasip1 --bin mhs-rust` build. Final 100M/32M sanity was `1704.445ms`, `59.15M` steps/s, `100,811,779` steps, `3` GCs, `192.992ms` GC pause, high-water `33,939,298`, sink `661902`, `cell_size_bytes=8`. No full self-host gate: this is cold socket/BFILE/test cleanup, not evaluator behavior |
| LZMA decoder module locality | accepted as crate-surface cleanup: moved the private checked raw-LZMA decoder from the crate root into `runtime/lzma_decode.rs`, removed the root `mod lzma_decode`, and updated `runtime/codecs.rs` to call it through the runtime module. Also fixed the stale platform module doc that still mentioned tracing after `MHS_WASI_TRACE` was removed. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --all-targets`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, and `git diff --check`. No benchmark gate: this is module locality/documentation only |
| post-simplification full self-host refresh | after the stale WASI trace trim, old makefile removal, cold clippy cleanup, and LZMA decoder module move, current default release at the 128M speed window completed 3x byte-identically: `56.086s`, `55.855s`, `55.869s` (median `55.869s`, avg `55.937s`), `3,659,074,793` steps, `31` GCs, GC pauses `8.710s`, `8.684s`, `8.754s` (median `8.710s`, avg `8.716s`), high-water `138,068,138`, sink `661902`, `cell_size_bytes=8`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp=0` for all three. Reading: source remains in the mid-56s band; this is a refresh/neutrality gate, not a claimed speed change from cleanup |
| rejected duplicate zero-arity FFI arm trim | tried deleting the apparently-unreachable zero-arity arms from the main `ffi_call` match (`errno_constant`, `host_constant`, `GETRAW`, time/platform/sizeof/want flags, `&closeb`, `&free`, `errno`) because `zero_arity_ffi_result` returns them before the arity-gated match can run; also tried the tiny `ffi_arity(&name)` borrow cleanup. Checks passed (`cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo test --lib` 43/43), but the 8-run 100M/32M source A/B rejected it hard: base avg `1780.360ms` versus candidate avg `2032.805ms` (`+14.18%`), same `100,811,779` steps, `3` GCs, high-water `33,939,302`, sink `661902`. Source was reverted. Reading: keep the duplicate-looking FFI match arms; they appear to help release codegen/layout enough that the source cleanup is not neutral |
| BFILE allocation-pointer range cleanup | accepted as small host-path cleanup: replaced three manual allocation-pointer bounds checks in `program/bfile/pointers.rs` with explicit `(ALLOCATION_PTR_BASE..0).contains(&ptr)` predicates. Verification passed `cargo fmt --check`, `cargo check --all-targets`, clippy `--all-targets --all-features` with the known unrelated warning groups suppressed, release `mhs-rust-bench` build, and a single 100M/32M sanity: `1.745s`, `57.77M` steps/s, `100,811,779` steps, `3` GCs, `193.702ms` GC pause, high-water `33,939,286`, sink `661902`, `cell_size_bytes=8`. No full self-host gate: this is BFILE pointer predicate cleanup, not evaluator, allocator, GC, or dispatch behavior |
| moving-GC lint surface cleanup | accepted as tiny source cleanup: removed stale module-level `allow(dead_code)` attributes from the feature-gated moving-GC remap/evacuation modules now that tests and the `moving-gc` feature both exercise their entrypoints, and kept only the standalone cold `ffi_arity(name)` borrow cleanup that survived the rejected duplicate-FFI-arm trim. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features moving-gc`, `cargo test --lib` 43/43, and `git diff --check`. No benchmark gate: this changes lint/source surface only and leaves default evaluator, allocator, GC, and FFI dispatch behavior unchanged |
| raw known-dispatch retest | rejected after retesting the rev-3/rev-5 decode-collapse idea on the current `codegen-units=1`/ThinLTO tree: `stack_eval_step` decoded `KnownPrim`/`RuntimePrim` directly from packed cell tag bits and moved FFI/JS cold arms behind the raw-tag branch, while preserving old runtime-primitive fallback semantics. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo test --lib` 43/43, and an isolated release bench build. 100M/32M A/B looked good but misleading: base avg `1944.781ms` versus candidate avg `1890.875ms` (`-2.77%`), same `100,811,779` steps, `3` GCs, high-water `33,939,312`, sink `661902`. Paired 3x full self-host rejected it: base `62.928s`, `62.472s`, `61.042s` (median `62.472s`) versus candidate `64.637s`, `60.716s`, `63.075s` (median `63.075s`, `+0.97%`), byte-identical SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, same `3,659,074,926` steps, `31` GCs, high-water `138,068,152`, sink `661902`. Source was reverted; treat dispatch decode as closed unless a future design changes heap/layout rather than just removing the wrapper |
| rev-5 profile cfg-gate | accepted as both simplification and a real speed win: added a `profile` cargo feature, made `eval-phase-profile` depend on it, cfg-gated `Program.profile`, `EvalProfile`, `enable_profile`/`take_profile`, and the real profile hooks behind that feature, and made default `ProfileHead` a zero-sized marker so strict/stack frames no longer carry an `Option<NodeId>` payload in normal builds. The bench and CLI `--profile` paths now fail early in default builds and work under `--features profile`; profile-enabled smoke printed the expected counters. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features profile`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc`, `cargo check --all-targets`, `cargo check --all-targets --features profile`, `cargo check --all-targets --features eval-phase-profile`, `cargo test --lib` 43/43, `cargo test --lib --features profile` 43/43, `cargo test --lib --features moving-gc` 43/43, release `mhs-rust-bench` default/profile/eval-phase-profile builds, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, default `--profile` error-path smoke, profile-enabled `--profile` smoke, final release bench build, and `git diff --check`. 100M/32M A/B against committed `abc45af0`: base avg `1753.974ms`, candidate avg `1672.473ms` (`-4.65%`), same `100,811,779` steps, `3` GCs, high-water `33,939,314`, sink `661902`; a final rebuilt-candidate 100M sanity after the CLI error-path polish was `1749.246ms`, same steps/sink/GC shape. Paired 3x full self-host accepted it: baseline `57.850s`, `57.426s`, `57.380s` (median `57.426s`, avg `57.552s`) versus candidate `54.648s`, `54.757s`, `54.733s` (median `54.733s`, avg `54.713s`, `-4.69%` median), byte-identical SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, same `3,659,074,945` steps, `31` GCs, high-water `138,068,154`, sink `661902`. Reading: rev 5's S1 target was correct; default runtime was still paying for disabled profiling through frame payload/layout and dataflow, and cfg-gating it is the new current best |
| bench `main_input_sink` alias removal | accepted as tiny bench-helper cleanup from rev 5 S3: deleted the `main_input_sink(input)` forwarding alias and used `bytes_sink(input)` directly in the step-limit/`main` fallback paths. Verification passed `cargo fmt --check`, `cargo check --all-targets`, release `mhs-rust-bench` build, and source grep confirmed no `main_input_sink` references remain. No benchmark gate: this is bench helper source cleanup and does not touch runtime evaluator behavior |
| Program `resolve` visibility narrowing | accepted as rev 5 S3 API-surface cleanup: narrowed `Program::resolve` from public API to `pub(in crate::runtime)` after source grep found only internal runtime/test callers. Behavior and codegen are unchanged; this just stops exposing an internal checked indirection resolver through the crate's exported `Program`. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo check --all-targets --features moving-gc`, and `cargo test --lib` 42/42. No benchmark gate: compile-time visibility only |
| over-fragmented eval module merge | accepted as rev 5 S3 source-organization cleanup: removed the one-purpose `eval/spine.rs` and `eval/fallback.rs` modules by moving eval-spine helpers into `eval/reduce.rs` and the fallback runtime-primitive rewrite helper into `eval/dispatch.rs`. No evaluator logic changed. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features eval-phase-profile`, `cargo check --features moving-gc`, `cargo check --all-targets`, `cargo test --lib` 43/43, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, and a bounded 100M/32M self-host sanity (`1.650s`, `61.11M` steps/s, `100,811,779` steps, `3` GCs, `193.741ms` GC pause, high-water `33,939,292`, sink `661902`, `cell_size_bytes=8`). Because this moved helpers across modules, it also got a same-session paired full gate against `01c0fe74`: baseline `54.173s`, `53.697s`, `53.647s` (median `53.697s`, avg `53.839s`) versus candidate `53.636s`, `53.501s`, `53.813s` (median `53.636s`, avg `53.650s`, `-0.11%` median), byte-identical SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, same `3,659,074,926` steps, `31` GCs, high-water `138,068,148`, sink `661902`. Reading: keep the merge; the earlier slow app-site baseline was ambient noise, not a module-merge regression |
| rejected app-site profile cfg split | tried making site-keyed app allocation profile-only after the `profile` feature gate: default `app_site!` macros called plain `app`, `app_with_site` existed only under `feature = "profile"`, and the default no-op `app_alloc_bookkeeping_cold` disappeared. Checks passed (`cargo fmt --check`, default `cargo check`, `cargo check --features profile`, `cargo check --features eval-phase-profile`). The 8-run 100M/32M A/B against `67d90850` was noisy and not clearly neutral: baseline avg `1773.795ms`, candidate avg `1788.979ms` (`+0.86%`), though medians favored the candidate (`1805.010ms` vs `1743.771ms`); all runs had identical `100,811,779` steps, `3` GCs, high-water `33,939,310`, and sink `661902`. Paired 3x full self-host rejected it: baseline `58.822s`, `57.092s`, `57.446s` (median `57.446s`, avg `57.787s`) versus candidate `58.507s`, `57.753s`, `57.949s` (median `57.949s`, avg `58.070s`, `+0.87%` median), byte-identical SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, same `3,659,074,945` steps, `31` GCs, high-water `138,068,150`, sink `661902`. Source was reverted; keep the existing `app_with_site` shape despite the apparent default no-op, because this split perturbs full-workload codegen negatively |
...
| profiling/diagnostics | default `EvalProfile` stores string/count maps; `ProfileHead` is transient `Option<NodeId>` in frames. The old `gc_young_profile_allocated_slots` R1 viability counter was retired from `gc-phase-profile` | remap transient `ProfileHead` through frame walkers. Keep profiling counters short-lived and delete them once the decision is made; do not let profile-only ids become a disabled-feature tax |
...
| historical full-heap evacuation scaffold | was implemented behind `cfg(any(test, feature = "moving-gc"))` to prove the shared remapper could compact an already-marked heap and update heap edges, cold payloads, program roots, BFILE read-only memory roots, node-pointer slots, eval spines, persistent spines, scratch roots, and machine stack roots. This was a correctness bridge only; the real `moving-gc` feature path later moved to nursery evacuation. Rev-5 simplification removed the dead full-heap helper and its dedicated test after P5 closed the standalone hot-setter/card R1 line |
| full-heap moving collector wiring | done behind `--features moving-gc`: the real evaluator GC call sites now pass mutable roots to `maybe_collect_moving_garbage_between_steps`, which reuses the current mark/weak/finalizer pass, conservatively marks `node_pointers` so guest pointer integers remain valid, runs dead foreign finalizers, queues weak finalizers before remapping, calls the full-heap evacuation/remapper, resets allocation cadence, and records normal GC stats/events. Default builds still compile the old mark-sweep call path. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features moving-gc`, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc,gc-phase-profile`, `cargo check --all-targets`, `cargo check --all-targets --features moving-gc`, `cargo test --lib` 43/43, `cargo test --lib --features moving-gc` 43/43, release `mhs-rust-bench --features moving-gc` build, default release `mhs-rust-bench` rebuild, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, and `git diff --check`. Forced 50k self-host slice with `MHS_GC_NODE_INTERVAL=1024` ran `38` moving collections, `69.137ms`, `50,860` steps, `62.432ms` GC pause, compact current arena `151,736` cells, high-water `154,343`, sink `661902`. Feature 100M/32M sanity survived but is intentionally not accepted for speed: `2.736s`, `36.84M` steps/s, `100,811,779` steps, `3` moving GCs, `729.125ms` GC pause, high-water `33,939,737`, current arena `16,749,311`, sink `661902`. Rebuilt default release 100M/32M sanity stayed in band at `1.750s`, `57.60M` steps/s, `3` mark-sweep GCs, `197.294ms` GC pause, high-water `33,939,302`, sink `661902`. Reading: full-heap moving proves the root/fixup path under real evaluator traffic and compacts aggressively, but full-heap copy is slower than tombstone sweep on bounded slices; the next R1 step must narrow this to nursery-only evacuation, not optimize full-heap evacuation as the final collector |
| post-moving default full self-host refresh | after committing the feature-gated collector wiring and rebuilding the normal default release, the 128M speed-window full gate completed 3x byte-identically: `57.065s`, `56.166s`, `56.753s` (median `56.753s`, avg `56.661s`), `3,659,074,869` steps, `31` mark-sweep GCs, GC pauses `8.614s`, `8.649s`, `8.685s` (median `8.649s`, avg `8.649s`), high-water `138,068,146`, sink `661902`, `cell_size_bytes=8`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp=0` for all three. Reading: default release remains in the mid-56s band; the `moving-gc` feature path is a correctness/profiling bridge, not a default-speed improvement |
| conservative nursery moving prototype | done behind `--features moving-gc`: added a `gc_nursery_start` suffix, made feature allocations bump-only so the nursery stays contiguous, marked direct young roots plus all old-space old-to-young edges by conservative scan, traced only young objects, and evacuated only marked young cells while keeping old `NodeId`s stable. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features moving-gc`, `cargo check --all-targets --features moving-gc`, `cargo test --lib` 43/43, `cargo test --lib --features moving-gc` 43/43, release `mhs-rust-bench --features moving-gc` build, default release `mhs-rust-bench` rebuild, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, SHA checks, and `git diff --check`. Forced 50k/1024 self-host slice improved versus full-heap moving: `33.914ms`, `50,860` steps, `38` nursery GCs, `26.567ms` GC pause, current/high-water `170,373`, sink `661902`. Feature 100M/32M sanity was correct but slow: `2.692s`, `37.45M` steps/s, `3` GCs, `752.787ms` GC pause, current `17,235,781`, high-water `34,225,308`, sink `661902`. Full feature self-host at 32M was byte-identical but rejected for speed: `131.950s`, `3,659,074,736` steps, `124` GCs, `67.534s` GC pause, current `82,063,523`, high-water `83,361,492`. Full feature self-host at the 128M speed window was byte-identical and better but still rejected: `110.283s`, `3,659,074,831` steps, `31` GCs, `41.154s` GC pause, current `62,200,807`, high-water `159,927,286`. Rebuilt default release 100M/32M sanity stayed in band at `1.689s`, `59.69M` steps/s, `3` mark-sweep GCs, `190.519ms` GC pause, high-water `33,939,318`, sink `661902`. Reading: correctness is proven for the nursery remap path and small forced-GC slices improve, but full self-host is dominated by scanning an ever-growing old space and by promoting too much into old space. The next R1 experiment needs a targeted remembered-set/card/dirty-line design or periodic old-space compaction; optimizing this conservative scan is the wrong target |
| rejected R1 precise remembered-source probe | tried replacing the conservative full old-space scan with a feature-gated exact old-source remembered set: setter hooks recorded old heap cells whose new payload pointed at the young suffix, with explicit coverage for direct `A.write` array mutation and `MVar` puts. Source was reverted after measurement, the default release bench was rebuilt, and post-revert 100M/32M sanity stayed in band at `1.726s`, `58.40M` steps/s, `3` mark-sweep GCs, `192.273ms` GC pause, high-water `33,939,316`, sink `661902`. Checks before the revert passed `cargo fmt`, default `cargo check`, `cargo check --features moving-gc`, `cargo check --all-targets`, `cargo check --all-targets --features moving-gc`, `cargo test --lib` 43/43, `cargo test --lib --features moving-gc` 43/43, and release `mhs-rust-bench --features moving-gc` build. Forced 50k/1024 improved versus the conservative scan: `18.545ms`, `50,860` steps, `38` GCs, `11.637ms` GC pause, current/high-water `170,383`, sink `661902`. But the 100M/32M slice regressed to `2.998s`, `33.62M` steps/s, `3` GCs, `800.797ms` GC pause, current `17,235,787`, high-water `34,225,314`, sink `661902`. Full feature self-host at 128M was byte-identical but worse than the conservative nursery: `116.810s`, `3,659,074,888` steps, `31` GCs, `42.318s` GC pause, current `62,200,878`, high-water `159,927,299`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp_exit=0`. Reading: the saved old-space scan is real on tiny forced-GC runs, but the branch/source-recording barrier in hot rewrite setters repeats the S4 failure and loses on self-host. Do not retry an exact per-write remembered set as a standalone R1 fix |
| rejected R1 coarse dirty-card probe | tried a feature-gated 1024-cell dirty-card table for old-to-young source scanning: the four heap setters dirtied old cards without exact payload checks, and cold array/MVar writes dirtied their owner cards. Source was reverted after measurement because it did not beat the conservative nursery; the default release bench was rebuilt and post-revert 100M/32M sanity stayed in band at `1.850s`, `54.49M` steps/s, `3` mark-sweep GCs, `200.165ms` GC pause, high-water `33,939,310`, sink `661902`. Checks before revert passed `cargo fmt --check`, default `cargo check`, `cargo check --features moving-gc`, `cargo test --lib` 43/43, `cargo test --lib --features moving-gc` 43/43, and release `mhs-rust-bench --features moving-gc` build. Forced 50k/1024 matched the exact remembered-source tiny win: `18.756ms`, `50,860` steps, `38` GCs, `11.914ms` GC pause, current/high-water `170,383`, sink `661902`. But 100M/32M regressed versus the conservative nursery: `2.798s`, `36.04M` steps/s, `3` GCs, `741.018ms` GC pause, current `17,235,787`, high-water `34,225,314`, sink `661902`. Full feature self-host at 128M was byte-identical but still worse than the conservative nursery: `110.427s`, `3,659,074,793` steps, `31` GCs, `40.009s` GC pause, current `62,200,717`, high-water `159,927,320`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp_exit=0`. Reading: coarse cards reduce mark-side scanning enough to lower total GC pause slightly, but the old-cell branch in hot setters still costs too much and the full gate is a tie/slight loss. Do not retry hot-setter card dirtying standalone |
| rejected R1 branchless dirty-card probe | tried the rev-5 P5 version of dirty cards: a feature-gated 1024-cell card byte vector, grown on allocation and reset after nursery evacuation, with an unconditional dirty-byte store in the four heap overwrite setters and old-space source scanning replaced by dirty-card scanning. Source was reverted after measurement. Checks before revert passed `cargo fmt --check`, default `cargo check`, `cargo check --features moving-gc`, `cargo check --features moving-gc,gc-phase-profile`, `cargo test --lib` 43/43, `cargo test --lib --features moving-gc` 43/43, and release `mhs-rust-bench --features moving-gc` build; after revert, `cargo fmt --check` and `cargo check --features moving-gc` passed. The 8-run moving-GC 100M/32M A/B rejected it before a full gate: conservative nursery baseline avg `2436.309ms`, median `2407.182ms`, avg GC `706.014ms`; branchless-card candidate avg `2563.717ms`, median `2577.883ms`, avg GC `685.357ms`; wall deltas were `+5.23%` avg and `+7.09%` median, with identical `100,811,779` steps, `3` GCs, high-water `34,225,316`, current arena `17,235,789`, and sink `661902`. Reading: even without an old-ness branch, the extra store in hot rewrites costs more than the saved old-space scan on the self-host slice; this closes standalone hot-setter card marking for R1 |
| R1 old-to-young source-shape profile | temporary `moving-gc,gc-phase-profile` diagnostic, reverted after measurement, scanned old space before each conservative nursery collection and classified old cells with young outgoing edges. 100M/32M slice stayed byte-shaped (`100,811,779` steps, `3` GCs, high-water `34,225,316`, current arena `17,235,789`, sink `661902`) and printed per-GC old→young sources: collection 1 `2242` sources / `2512` edges (`app=1318/1588`, `indir=924/924`), collection 2 `2300` / `2961` (`app=1984/2645`, `indir=316/316`), collection 3 `2207` / `3150` (`app=1871/2814`, `indir=336/336`). No cold array/MVar/pointer/weak sources appeared in this slice. Reading: cold-path-only remembered tracking is not enough; the remaining old→young problem is app/indir update traffic, so the next R1 design must avoid a hot setter tax while still accounting for old App/Indir rewrites |
| accepted R1 sparse nursery remap | changed the `moving-gc` nursery evacuator to keep old-space IDs identity-mapped and store only live young survivor remaps, replacing the previous whole-heap `Vec<Option<NodeId>>` table. This is feature-only and does not change default mark-sweep release code. Verification passed `cargo fmt --check`, `cargo check --features moving-gc`, `cargo test --lib --features moving-gc` 42/42, and release `mhs-rust-bench --features moving-gc` build. Same-session 100M/32M feature baseline before the patch was `2523.342ms`, `39.95M` steps/s, `3` GCs, `763.494ms` GC pause, high-water `34,225,318`, current arena `17,235,791`, sink `661902`. Candidate was `2076.269ms`, `48.55M` steps/s, identical `100,811,779` steps and `3` GCs, `320.265ms` GC pause, high-water `34,225,326`, current arena `17,235,799`, sink `661902`. Full 128M feature self-host was byte-identical at `77.342s`, `3,659,074,907` steps, `47.31M` steps/s, `31` GCs, `16.175s` GC pause, high-water `159,927,318`, current arena `62,200,887`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, output size `661,784`. Reading: the whole-heap remap table was a large moving-GC tax, but the feature path is still slower than default mark-sweep; next R1 work must reduce promotion/old-space scan behavior or change nursery shape, not pay hot setter/card costs |
| accepted R1 selected heap remap | changed the `moving-gc` old-space scan to record old cells that actually point into the nursery, then made nursery evacuation remap only those old sources plus moved young survivors and non-heap roots/stacks. The full test remapper remains for coverage; the active nursery path no longer walks every old heap cell/cold payload just to apply mostly-identity remaps. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features moving-gc`, `cargo test --lib --features moving-gc` 42/42, and release `mhs-rust-bench --features moving-gc` build. The 100M/32M slice was neutral versus sparse remap (`2076.269ms` / `320.265ms` GC before, `2059.620ms` / `322.616ms` GC after; identical `100,811,779` steps, `3` GCs, and sink `661902`) because old space is still small there. The full 128M feature gate improved modestly in 3x samples: `76.228s`, `76.775s`, `76.741s` (median `76.741s`, avg `76.581s`), GC pauses `15.876s`, `16.319s`, `15.903s` (median `15.903s`, avg `16.033s`), byte-identical SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, high-water about `159,927,278` cells, current arena about `62,201,039`, and sink `661902`. Reading: this is the right kind of R1 cleanup, with no hot setter tax, but it is not the larger win; old-space scanning/promotion still dominate enough that the feature path remains far behind default mark-sweep |
| rejected R1 periodic old compaction | tried restoring the full-heap moving evacuation bridge as an env-tunable `moving-gc` periodic compaction (`MHS_MOVING_GC_FULL_PERIOD`) so nursery collection could occasionally reclaim dead old objects. Source was reverted after measurement. Correctness held: both tested full runs produced SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a` and sink `661902`. Period 8 reduced memory materially (`gc_current_nodes` `62.2M -> 45.6M`, high-water `159.9M -> 143.5M`) but regressed wall to `80.659s` versus selected-remap median `76.741s`; period 16 also reduced memory (`current 52.7M`, high-water `150.4M`) but regressed further to `81.477s`. Reading: old garbage is real, but copying the whole live old set is too expensive as a periodic side operation; do not revive full-heap compaction unless the heap representation makes evacuation much cheaper or replaces nursery collection entirely |
| rejected stack raw-code dispatch quick retest | retried the NOTES decode-collapse idea only in `stack_eval_step`: raw known-prim code constants plus a stack-local head enum, leaving fallback/persistent paths decoded. Source was reverted after the 100M/32M slice landed in the current noise band: `1693.766ms`, `59.52M` steps/s, identical `100,811,779` steps, `3` GCs, `196.089ms` GC pause, high-water `33,939,304`, and sink `661902`. Verification before revert passed `cargo fmt --check`, default `cargo check`, `cargo test --lib` 42/42, and release bench build. Reading: current ThinLTO/codegen-units=1 builds still do not get a standalone win from removing the `KnownPrim` decode wrapper; keep dispatch-decode closed unless it comes with a broader evaluator representation change |
| current GC phase-profile refresh | zero-code `gc-phase-profile` 100M/32M self-host slice after the current simplification/performance series: total `1682.218ms`, `100,811,779` steps, `59.93M` steps/s, `3` GCs, `197.224ms` total GC pause, `65.708ms` mark, `103.548ms` sweep, high-water `33,939,300`, sink `661902`. Per-collection sweep stayed about `33.7ms`/`34.7ms`/`35.1ms`, while mark was `19.6ms`/`18.6ms`/`27.6ms`. Reading: R5's obvious sweep fast path is already folded in; remaining sweep cost is the arena scan/free-list rebuild shape, not an isolated local branch to trim |
| dead full-heap moving scaffold trim | rev-5 simplification target accepted: removed the test-only `evacuate_marked_heap_for_moving_gc` full-heap evacuation helper and its dedicated coverage test. The active `moving-gc` feature path already uses `evacuate_marked_nursery_for_moving_gc`, and the shared `NodeId` remapper coverage remains. Default performance risk is zero because the removed helper was gated behind `cfg(any(test, feature = "moving-gc"))` and had no production callers; `moving-gc` keeps the nursery suffix/evacuation machinery for any future R1 design. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features moving-gc`, `cargo test --lib` 42/42, `cargo test --lib --features moving-gc` 42/42, release `mhs-rust-bench` build, `git diff --check`, and 100M/32M bounded self-host sanity (`1675.830ms`, `60.16M` steps/s, `100,811,779` steps, `3` GCs, `193.761ms` GC pause, high-water `33,939,292`, sink `661902`, `cell_size_bytes=8`) |
| moving-GC evacuation module gate | accepted as follow-up simplification after the full-heap helper/test deletion: `runtime/program/evacuate.rs` now compiles only under `--features moving-gc`, and the nursery evacuation entrypoint no longer needs a dead-code allowance for default test builds. Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features moving-gc`, `cargo test --lib` 42/42, and `cargo test --lib --features moving-gc` 42/42. No benchmark gate: this removes default-test compile surface only, not default release code |
| stale GC young-profile trim | accepted as performance-neutral simplification: removed the old S4/R1 young-region viability counters from `gc-phase-profile`, including allocation-slot recording, old-to-young edge counting, stats/event fields, remap/evacuation hooks, and bench output. Kept the useful GC phase timers (`mark`/`sweep`). Verification passed `cargo fmt --check`, default `cargo check`, `cargo check --features gc-phase-profile`, `cargo check --features moving-gc,gc-phase-profile`, `cargo check --all-targets`, `cargo test --lib` 43/43, `cargo test --lib --features moving-gc` 43/43, release `mhs-rust-bench` build, and source grep for stale young-profile symbols. Final 100M/32M sanity was `1731.145ms`, `58.23M` steps/s, `100,811,779` steps, `3` GCs, `190.537ms` GC pause, high-water `33,939,318`, sink `661902`, `cell_size_bytes=8`. Full 128M gate completed 3x byte-identically at `56.364s`, `55.941s`, `56.133s` (median `56.133s`, avg `56.146s`), `31` GCs, median GC pause `8.678s`, high-water `138,068,158`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp=0` |
| dead moving-GC scaffold deletion | accepted as performance-neutral simplification after rev-7 closed R1: removed the `moving-gc` cargo feature, `program/remap.rs`, `program/evacuate.rs`, the nursery collector/young-marking helpers, `gc_nursery_start`, feature-only bump-allocation branches, mutable-root GC safe-point forks, and the moving-GC remapper test. Active default GC is back to a single mark/sweep path. Verification passed `cargo fmt --check`, `cargo check`, `cargo check --all-targets`, `cargo check --features gc-phase-profile`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, broad source grep for stale moving-GC symbols, and `git diff --check`. Final 100M/32M sanity was `1803.660ms`, `55.89M` steps/s, `100,811,779` steps, `3` GCs, `206.976ms` GC pause, high-water `33,939,300`, sink `661902`, `cell_size_bytes=8`. Full 128M gate completed 3x byte-identically at `55.808s`, `55.041s`, `55.217s` (median `55.217s`, avg `55.355s`), `3,659,075,002` steps, `31` GCs, median GC pause `8.771s`, high-water `138,068,160`, sink `661902`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, output size `661,784`. Reading: deleting the rejected feature path and cfg forks is neutral in the current full-gate band |
| stale eval-phase-profile deletion | accepted as performance-neutral simplification after performance probing closed: removed the `eval-phase-profile` cargo feature, stack phase timers, stack step/head arity/transition counters, resolved app-allocation shape profiling, allocation reuse/fresh timing counters, shape helper functions, output sections, and their hot-path cfg blocks. Kept the basic `profile` feature and `gc-phase-profile` mark/sweep timers. Verification passed `cargo fmt --check`, `cargo check`, `cargo check --all-targets`, `cargo check --features profile`, `cargo check --features gc-phase-profile`, `cargo check --features profile,gc-phase-profile`, `cargo check --all-targets --features profile`, `cargo test --lib` 41/41, release `mhs-rust-bench` default/profile builds, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, 1M profile smoke (`27.972ms` normal, `149.346ms` profiled, sink `661902`) with no retired phase/shape sections, and `git diff --check`. Final 100M/32M sanity was `1681.489ms`, `59.95M` steps/s, `100,811,779` steps, `3` GCs, `190.836ms` GC pause, high-water `33,939,314`, sink `661902`, `cell_size_bytes=8`. No full gate: this deletes retired non-default profiling code; default runtime behavior and heap shape stayed in the bounded sanity band |
| profile-head guard cleanup | accepted as performance-neutral simplification: removed redundant `ProfileHead::is_some()` guards before `profile_reduction` in stack, persistent, fallback, strict-frame, and dispatch reducers, then deleted the now-unused method. `profile_reduction` already returns immediately for empty heads in `--features profile`, and the default build hook is inline no-op, so behavior is unchanged while source boilerplate drops. Verification passed `cargo fmt --check`, `cargo check`, `cargo check --all-targets`, `cargo check --features profile`, `cargo test --lib` 41/41, default/profile release `mhs-rust-bench` builds, 1M profile smoke (`29.070ms` normal, `146.980ms` profiled, sink `661902`, `profile_reductions=1,001,388`), `git diff --check`, and a 100M/32M bounded sanity (`1839.638ms`, `54.80M` steps/s, `100,811,779` steps, `3` GCs, `201.268ms` GC pause, high-water `33,939,312`, sink `661902`, `cell_size_bytes=8`). Full 32M self-host also completed byte-identically at `65.640s`, `3,659,074,793` steps, `124` GCs, `20.530s` GC pause, high-water `39,192,408`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, `cmp=0` |
| dead persistent/non-stack reducer deletion | accepted as performance-neutral simplification: removed the unreachable persistent-spine reducer (`eval/persistent.rs`), the unused `whnf_frames`/`reduce_whnf_from_inner` non-stack WHNF driver, non-stack `EvalFrameStack`/`StrictRedex` state, persistent-spine GC roots, and the old strict-marker mode in the fallback reducer. The active reducer is now the stack WHNF driver plus the delegated fallback eval loop only. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo check --features profile`, `cargo check --features gc-phase-profile`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, broad source grep for stale persistent/non-stack frame symbols, and `git diff --check`. Final 100M/32M sanity was `1718.091ms`, `58.68M` steps/s, `100,811,779` steps, `3` GCs, `198.408ms` GC pause, high-water `33,939,314`, sink `661902`, `cell_size_bytes=8`. Full 128M gate completed 3x byte-identically at `54.345s`, `55.106s`, `55.355s` (median `55.106s`, avg `54.935s`), `3,659,074,945` steps, `31` GCs, median GC pause `8.883s`, high-water `138,068,154`, sink `661902`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, output size `661,784`, `cmp=0` for all three. Reading: this supersedes the old standalone persistent-dispatch deletion warning; with the current stack driver/code shape the deletion is neutral in the mid-50s full-gate band |
| profile counter rename after persistent deletion | accepted as source clarity cleanup: renamed the remaining live `persistent_*` profile fields/hooks/output labels to `strict_forces` and `fallback_entries`, matching the active stack-frame/fallback-entry behavior now that the persistent reducer is gone. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo check --features profile`, profile release `mhs-rust-bench` build, 1M profile smoke (`27.580ms` normal, `143.204ms` profiled, `profile_steps=1,001,388`, sink `661902`, `profile_strict_forces=35,793`, `profile_fallback_entries=90`), default release `mhs-rust-bench` rebuild, broad source grep for stale live `persistent` profile names, and `git diff --check`. No self-host gate: this only renames profile feature counters and labels |
| architecture docs and core invariant comments | accepted as performance-neutral simplification/documentation: added `rust/microhs-runtime/ARCHITECTURE.md`, linked it from `rust/README.md`, and documented the packed `Cell`, stable `NodeId`, `Program` state owner, explicit `EvalStack`, stack reducer entry, descent, and between-step mark-sweep collection. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo test --lib` 41/41, `cargo doc --no-deps`, and `git diff --check`. No self-host gate: docs/comments only |
| zero-arity FFI dispatch trim | accepted as performance-neutral simplification: removed unreachable duplicate zero-arity FFI arms from the main `ffi_call` dispatch. Those constants and function pointers already return through `zero_arity_ffi_result` before the arity gate when a continuation arg is present; stateful zero-arity operations such as `new_mpz`, `environ`, `get_executable_path`, and `openb_wr_mem` remain in the main dispatch. Verification passed `cargo fmt`, `cargo check --all-targets`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, `git diff --check`, and 100M/32M bounded self-host sanity (`1676.905ms`, `60.12M` steps/s, `100,811,779` steps, `3` GCs, `201.502ms` GC pause, high-water `33,939,304`, sink `661902`, `cell_size_bytes=8`) |
| unary-math FFI dispatch trim | accepted as performance-neutral simplification: removed duplicate one-argument math FFI arms from the main `ffi_call` dispatch. `unary_math_ffi_result` already handles `acos`/`sin`/`sqrt` and their float variants before the arity gate when the continuation arg is present; binary math (`atan2`, `pow`, `scalbn` and `f` variants) remains in the main dispatch. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, `git diff --check`, and 100M/32M bounded self-host sanity (`1640.548ms`, `61.45M` steps/s, `100,811,779` steps, `3` GCs, `191.335ms` GC pause, high-water `33,939,294`, sink `661902`, `cell_size_bytes=8`) |
| FFI pointer load/store helper cleanup | accepted as performance-neutral simplification: replaced the repeated `peek_*`/`poke_*` pointer load/store arm bodies in `runtime_dispatch/ffi.rs` with local helper methods while preserving the existing arity gate, normal pair return path, and special `peekWord`/`pokeWord`/`peek_uint64`/`poke_uint64` continuations. `ffi.rs` shrank from 1005 to 931 lines. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo check --features profile`, `cargo check --features gc-phase-profile`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, `cargo check --target wasm32-unknown-unknown`, `cargo check --target wasm32-wasip1 --bin mhs-rust`, `git diff --check`, and 100M/32M bounded self-host sanity (`1717.674ms`, `58.69M` steps/s, `100,811,779` steps, `3` GCs, `196.882ms` GC pause, high-water `33,939,324`, sink `661902`, `cell_size_bytes=8`). Noise-control full sample in the same window: C `-O3` at `134,217,728` cells was byte-identical in `42.08s` with `34` GCs and `4.61s` GC pause, versus prior same-cell C median `40.30s`; Rust 128M was byte-identical in `55.434s`, `3,659,074,888` steps, `31` GCs, `8.976s` GC pause, high-water `138,068,148`, same SHA/cmp. Reading: this is a clarity cleanup; the full sample is load-affected and not a new performance claim |
| runtime test module split | accepted as test-only source-organization cleanup: split the 816-line `runtime/tests.rs` into a 54-line helper/module root plus focused `tests/{arrays_io,bytes,exceptions,ffi,handles_gc,numeric,reduction,serialization}.rs` modules, largest 134 lines. Test count and namespacing stay the same semantically, and the old bigint wire-format local variable no longer uses misleading `legacy` wording. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo check --all-targets --features profile,gc-phase-profile`, `cargo test --lib` 41/41, source grep for stale `legacy`/`persistent`/`moving-gc`/`eval-phase-profile`/`WRAPBUG`/`include!` residue, and `git diff --check`. No self-host gate: test-only file movement |
| post-FFI simplification full refresh | current-source 128M full self-host after the profile rename, architecture docs, zero-arity FFI trim, and unary-math FFI trim completed 3x byte-identically at `54.163s`, `53.925s`, `54.420s` (median `54.163s`, avg `54.169s`), `3,659,074,888` steps, `31` GCs, median GC pause `8.708s`, avg GC pause `8.713s`, high-water `138,068,148`, sink `661902`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, output size `661,784`, `cmp=0` for all three. Reading: simplification remains in the established mid-50s 128M full-gate band |
| strict-action/test wrapper cleanup | accepted as performance-neutral simplification: replaced the repeated `RuntimePrim::strict_action` arity/op ladder with a local macro that expands to the same checks, and removed the redundant inner `mod tests` wrapper from `runtime/tests.rs`. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo check --features profile`, `cargo check --features gc-phase-profile`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, `cargo check --target wasm32-unknown-unknown`, `cargo check --target wasm32-wasip1 --bin mhs-rust`, `git diff --check`, and 100M/32M bounded self-host sanity (`1680.696ms`, `59.98M` steps/s, `100,811,779` steps, `3` GCs, `192.675ms` GC pause, high-water `33,939,344`, sink `661902`, `cell_size_bytes=8`). No full gate: the runtime change is macro-equivalent code cleanup and the test-wrapper edit is test-only |
| let-return cleanup | accepted as performance-neutral simplification: removed redundant local return bindings from `push_node`, `push_app_node`, and `apply_stack_app`, plus the needless host-constant `return`. Verification passed `cargo fmt --check`, `cargo check --all-targets`, `cargo check --features profile`, `cargo check --features gc-phase-profile`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, `git diff --check`, and 100M/32M bounded self-host sanity (`1679.622ms`, `60.02M` steps/s, `100,811,779` steps, `3` GCs, `192.524ms` GC pause, high-water `33,939,351`, sink `661902`, `cell_size_bytes=8`). No full gate: this only removes redundant locals/return syntax and stays in the same bounded sanity band |
| post-cleanup full self-host refresh | after `f3e8b070` and `7e9289c0`, current-source 128M full self-host completed 3x byte-identically at `55.443s`, `54.289s`, `54.221s` (median `54.289s`, avg `54.651s`), `3,659,074,945` steps, `31` GCs, GC pauses `8.846s`, `8.698s`, `8.778s` (median `8.778s`, avg `8.774s`), high-water `138,068,212`, sink `661902`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, output size `661,784`, `cmp=0` for all three. Reading: the cleanup series remains in the established mid-50s 128M full-gate band |
| R1 moving-GC line | retired for now. The feature path proved root/fixup correctness but lost full self-host badly, and the follow-up remembered-source/card/periodic-compaction probes all failed. Do not revive hot setter barriers, dirty cards, conservative old-space scans, or full-heap copy cleanup as standalone work; any future collector attempt should start from a new heap/nursery design rather than this deleted scaffold |
| avoid | do not optimize the removed conservative old-space scan as if it were the final collector. The useful lesson is that copying/root-fixup was correct but too expensive without a fundamentally different collector shape |

## 2026-07-05 Current 8B Fair-Memory Interval Refresh

| item | result |
|---|---|
| setup | zero-code full self-host refresh after the rev-5 profile cfg-gate and eval-module merge. Built current release `mhs-rust-bench` and ran `MHS_GC_NODE_INTERVAL=83886080` (80Mi allocation window), `MHS_STEP_LIMIT=none`, `timeout 900s`, three samples. This is the current fairer-memory point: high-water is `88,151,910` 8-byte cells (~705MB of cell arena, plus mark/cold/vector overhead), much closer to eval.c's 800MB heap than the 128M speed setting |
| 80Mi interval 3x | byte-identical outputs (`cmp=0`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`) at `57.026s`, `55.926s`, `55.516s`; median `55.926s`, average `56.156s`. All samples used `3,659,074,907` steps, `49` GCs, high-water `88,151,910` cells, sink `661902`, and `cell_size_bytes=8`. GC pause samples were `11.095s`, `11.147s`, `11.053s`; median `11.095s`, average `11.098s` |
| reading | The stale high-60s fair-memory headline is obsolete after the profile cfg-gate. At roughly C-sized memory, current Rust is now `55.926s`, about `1.17x` the fresh C oracle (`47.85s`) while using 49 GCs and ~705MB of cell arena. The 128M speed setting remains faster at `53.636s`, but only by ~4.1% while using a larger arena (`138.07M` 8-byte cells) and fewer GCs (`31` vs `49`). The remaining gap is still GC/locality: 80Mi spends ~11.1s in GC versus ~8.65s at 128M and C's 47.85s total oracle |

## 2026-07-05 Post-Simplification 128M Refresh
...
| item | reading |
|---|---|
| persistent-spine pure/IO alias path | deleted after the stack WHNF driver became the only live path. The old standalone deletion warning is obsolete: current 3x 128M full gate is byte-identical at median `55.106s`, in the same mid-50s simplification band |
...
| priority | work |
|---|---|
| 1 | Performance probing is closed after the rev-7 Rust PGO floor check plus the A.2 fair C PGO/matched-heap matrix and A.3 80M matched-memory PGO column. Do not chase M1/M2 or more R1/dispatch variants unless they fall out of simplification for free |
| 2 | Done: deleted the dead `moving-gc` scaffold (`moving-gc` feature, remap/evacuate modules, nursery collector fork, feature-only allocation/GC branches, and moving-GC tests) now that R1 went negative and PGO quantified the floor |
| 3 | Done: removed the stale `eval-phase-profile` performance-probe layer while keeping basic `profile` and `gc-phase-profile` support |
| 4 | Done: deleted the dead persistent-spine/non-stack WHNF driver and its GC root plumbing; active reduction is now stack driver plus delegated fallback loop |
| 5 | Done: renamed stale live `persistent_*` profile counters after deleting the persistent reducer |
| 6 | Done: added architecture docs and core invariant comments for the reducer, 8-byte cell/cold side table, stable `NodeId`, explicit stack, and mark-sweep GC |
| 7 | Done: removed unreachable duplicate zero-arity FFI arms from the main dispatch |
| 8 | Done: removed duplicate one-argument math FFI arms from the main dispatch |
| 9 | Continue performance-neutral simplification: sweep remaining rejected-probe residue and unused cfg gates, then tighten module boundaries/naming where it removes real confusion |
| 10 | Guardrail for simplification: stay byte-identical and inside the neutrality band. Use `cargo fmt --check`, default/feature checks that remain, `cargo test --lib`, release bench build, wasm/browser/WASI builds for broad changes, and 8-run 100M/32M A/B plus paired 3x full self-host when touching evaluator, allocator, active GC roots, dispatch layout, or large runtime code shape |
