...
| 2 | Continue R1 from the proven `moving-gc` collector bridge only with designs that avoid touching every hot rewrite. The nursery suffix/remapper works and sparse young-survivor remapping removed a large feature-GC tax, but exact remembered sources, coarse dirty cards, and branchless dirty cards all lost to setter cost. The temporary source-shape profile found old→young edges in old App/Indir cells, not cold arrays/MVars/pointers, so cold-path-only tracking is insufficient. Preserve guest pointer integers by keeping `node_pointers` targets marked/remapped and rebuilding `node_pointer_slots`; next plausible R1 work is allocation-site-specific old App/Indir update handling, periodic old-space compaction, or a nursery shape that reduces promotion and does not require per-rewrite old-to-young tracking |
