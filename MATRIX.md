...
| local commits ahead after this snapshot commit | 60 |
| runtime code baseline | generalized combinator app-reuse checkpoint |
...
Last fully verified state: generalized combinator app-reuse checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed before generalized combinator app-reuse checkpoint commit; 27 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed before generalized combinator app-reuse checkpoint commit |
| `cargo check -p microhs-runtime --bins --quiet` | passed before generalized combinator app-reuse checkpoint commit |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before generalized combinator app-reuse checkpoint commit |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before generalized combinator app-reuse checkpoint commit |
...
| `cargo fmt --all --check` | passed before generalized combinator app-reuse checkpoint commit |
| `git diff --check` | passed before generalized combinator app-reuse checkpoint commit |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release --bin mhs-rust-bench --quiet` | passed before generalized combinator app-reuse checkpoint commit |
...
| `--help` main smoke | 10,810,951 ns/iter; sink `661902` | 104,514,392 ns/iter; sink `960903` | both run and print usage; not a full compile; refreshed with `--warmup-iters 1 --iters 3 --profile --profile-top 8`; Rust sink is runtime-internal nodes/steps, not a semantic comparison |
