...
| local commits ahead after this snapshot commit | 106 |
| runtime code baseline | F2 evaluator-owned WHNF spine-loop slice |
...
Last fully verified state: F2 evaluator-owned WHNF spine-loop checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed after F2 evaluator-owned WHNF spine-loop slice; 34 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed after F2 evaluator-owned WHNF spine-loop slice |
| `cargo check -p microhs-runtime --bins --quiet` | passed after F2 evaluator-owned WHNF spine-loop slice |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 evaluator-owned WHNF spine-loop slice |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 evaluator-owned WHNF spine-loop slice |
...
| `cargo fmt --all --check` | passed after F2 evaluator-owned WHNF spine-loop slice |
| `git diff --check` | passed after F2 evaluator-owned WHNF spine-loop slice |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed after F2 evaluator-owned WHNF spine-loop slice |
...
| self-host Rust main smoke | not yet passing; no-shim run no longer falsely invokes `cpphs`; latest refreshed full compile run hit `timeout 300s` with exit `124`, printed only the missing `mhs.conf` warning to `/tmp/mhs-selfhost-rust-300s-20260703-032246.out`, left empty stderr, and produced no output comb |
...
| `--help` main smoke | 12,341,422 ns/iter; sink `661902` per iter | 61,450,144 ns/iter; sink `661902` per iter | both run and print identical usage text; not a full compile; refreshed with `--warmup-iters 1 --iters 3` after the F2 evaluator-owned WHNF spine-loop slice; this is the first selected single-loop speedup, but the proxy is still about 5x C on this noisy machine |
| self-host compiler smoke | 56,387,928,187 ns/iter; output `/tmp/mhs-selfhost-c-refresh.comb` is a 647 KiB `v8.4` comb | latest refreshed full run hit `timeout 300s` with exit `124`; only `/tmp/mhs-selfhost-rust-300s-20260703-032246.out` warning was produced, stderr was empty, and no output comb existed | not at parity |
...
`target/release/mhs-rust-bench --input /tmp/mhs-selfhost.comb --mode main --warmup-iters 0 --iters 1 --profile --profile-top 20 -- ./bin/mhs --help > /tmp/mhs-selfhost-f2-loop-current-profile.out`
...
| measure | count | implication |
|---|---:|---|
| profiled iteration time | 115.648 ms | instrumentation overhead is separate from normal timing; raw time is noisy |
| reductions | 297,417 | F2 evaluator-owned spine loop preserves the current work count; speedup comes from loop mechanics, not changed semantics |
...
| profiled resolve calls | 5,064,884 | resolve/node-classification traffic is still much larger than reduction count, but the selected loop removes part of the normal-run overhead around it |
...
| Use marker continuations for strict primitive forcing | Binary/unary int, int64, float, double, and bytes primops push nodes such as `T_BININT2`/`T_BININT1` and finish in `ret`, with fast paths when operands are already boxed values | Rust now has cheap fast paths for already-WHNF coercion arguments plus explicit continuation stacks for nested Int, Int64 including mixed shifts, Float64, Float32, binary ByteString marker forcing, and the strict WHNF arms `seq`, `IO.strict`, and `isInt`; the main reducer loop still needs the same treatment | partial |
...
| F2 bridge lesson | Exposing the existing `IntFrame` helper stack through a `StepAction` request was not enough; it changed `arith-chain:200` from one outer WHNF step to 200 driver-counted steps and regressed the common arithmetic row despite matching sinks. The selected F2 slice instead moves the normal WHNF spine walk and hot reducers into the driver; strict marker unification remains the next step. |
...
| self-hosting parity | C runtime can execute the compiler comb and produce a new compiler comb | Rust main-mode harness exists and lazy file IO now matches the C CPP scan smoke; latest refreshed full-compile run still hit the 300s timeout with no output comb; inline spine work removed almost all `--help` heap-spine spills, generalized combinator app reuse cuts self-host `--help` node growth by ~27%, direct known-head/helper dispatch plus C-style `Y` sharing keep the short self-host proxy in the ~70-85 ms band, small-int caching trims final profile nodes to 615,854, strict WHNF coercion fast paths improved the proxy before noisy reruns, F2 numeric/ByteString/WHNF/rnf slices remove several native-recursion paths, and the F2 evaluator-owned WHNF spine-loop slice drops the compiler-shaped proxy to Rust `61,450,144` ns/iter vs C `12,341,422` with matching sink; full self-host parity still needs generic strict primops and helper evaluators merged into the marker-machine loop |
