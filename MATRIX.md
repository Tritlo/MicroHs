...
| delta | against current source medians, PGO 128M is `-15.2%` versus the `53.636s` best current-source gate and `-17.8%` versus the latest `55.306s` refresh. PGO 80Mi is `-11.6%` versus the current fair-memory `55.926s` median. Relative to the fresh C oracle `47.85s`, PGO 128M is faster (`0.95x C`) at the larger speed heap, while PGO 80Mi is `1.03x C` at roughly C-sized memory |
| reading | this quantifies the codegen/layout floor rather than a source win. PGO erases almost all of the remaining gap and confirms rev 7's thesis: remaining default-release performance is largely profile-guided layout/inlining/codegen, not an obvious source-level evaluator rewrite. Per directive, close performance probing here and pivot to review/simplification; do not ship PGO |
...
| priority | work |
|---|---|
| 1 | Performance probing is closed after the rev-7 PGO floor check. Do not chase M1/M2 or more R1/dispatch variants unless they fall out of simplification for free |
