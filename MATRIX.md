...
| local commits ahead after this snapshot commit | 99 |
| runtime code baseline | F2 numeric/bytes continuation-stack checkpoint |
...
Last fully verified state: F2 numeric/bytes continuation-stack checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed before F2 numeric/bytes continuation-stack commit; 34 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed before F2 numeric/bytes continuation-stack commit |
| `cargo check -p microhs-runtime --bins --quiet` | passed before F2 numeric/bytes continuation-stack commit |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before F2 numeric/bytes continuation-stack commit |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before F2 numeric/bytes continuation-stack commit |
...
| `cargo fmt --all --check` | passed before F2 numeric/bytes continuation-stack commit |
| `git diff --check` | passed before F2 numeric/bytes continuation-stack commit |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed before F2 numeric/bytes continuation-stack commit |
...
| F2 numeric/bytes continuation-stack probe | passed before checkpoint commit; `eval_int`, non-shift `eval_int64`, `eval_float64`, `eval_float32`, and binary ByteString marker ops now force nested strict primops through explicit inline-one-frame continuation stacks instead of recursively calling `reduce_node_whnf`; `arith-chain:200` is Rust `36,122` ns/iter vs C `108,853`, `int64-chain:200` is Rust `35,442` ns/iter vs C `112,619`, `float64-chain:200` is Rust `40,276` ns/iter vs C `120,763`, `float32-chain:200` is Rust `48,362` ns/iter vs C `111,033`, `bytes-chain:200` is Rust `49,465` ns/iter vs C `164,437`, and self-host `--help` is about `69.3 ms` on the 5-iter rerun; still a structural checkpoint rather than a full compiler-speed win |
...
Latest tracked runtime behavior change is the current F2 strict marker-continuation slice: `eval_int`, non-shift `eval_int64`, `eval_float64`, `eval_float32`, and binary ByteString marker ops handle nested strict primops with explicit continuation stacks instead of native recursive reducer calls.
...
| `--help` main smoke | 10,084,850 ns/iter; sink `661902` per iter | 69,344,781 ns/iter; sink `913271` per iter | both run and print identical usage text; not a full compile; refreshed with `--warmup-iters 1 --iters 5` after the F2 binary ByteString continuation-stack slice; byte-chain improves sharply, but this compiler-shaped proxy is roughly noisy/neutral versus the float slice and still needs broader reducer-loop work |
...
| Use marker continuations for strict primitive forcing | Binary/unary int, int64, float, double, and bytes primops push nodes such as `T_BININT2`/`T_BININT1` and finish in `ret`, with fast paths when operands are already boxed values | Rust now has cheap fast paths for already-WHNF coercion arguments plus explicit continuation stacks for nested Int, non-shift Int64, Float64, Float32, and binary ByteString marker forcing; mixed Int64 shifts and the main reducer loop still need the same treatment | partial |
