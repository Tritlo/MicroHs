...
| priority | work |
|---|---|
| 1 | Continue performance-neutral simplification, but only where it removes real surface area or stale profiling/runtime structure. Rev 5 S1 (`profile` feature cfg-gate) and the S3 tiny-module merge are done and accepted; do not spend more time shaving public API unless it deletes meaningful coupling |
| 2 | Continue R1 from the proven `moving-gc` collector bridge only with designs that avoid touching every hot rewrite. The nursery suffix/remapper works, sparse young-survivor remapping removed a large feature-GC tax, and selected heap remapping modestly improved the full feature gate, but exact remembered sources, coarse dirty cards, branchless dirty cards, and periodic full old compaction all lost. The temporary source-shape profile found old→young edges in old App/Indir cells, not cold arrays/MVars/pointers, so cold-path-only tracking is insufficient. Preserve guest pointer integers by keeping `node_pointers` targets marked/remapped and rebuilding `node_pointer_slots`; next plausible R1 work is allocation-site-specific old App/Indir update handling or a nursery shape that reduces promotion and does not require per-rewrite old-to-young tracking |
