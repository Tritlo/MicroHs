...
| item | state |
|---|---|
| branch | `microhs-rust`, local branch is ahead of `origin/microhs-rust`; `split-runtime-modules` was fast-forward merged and remains as the source-organization side branch |
...
| parity state | benchmark and smoke sinks match; Rust self-host compile produces byte-identical output. Current runtime uses unconditional 8-byte cells. Latest current-source 128M full self-host refresh after the stale per-head eval timing trim is 3x byte-identical at `56.419s`, `56.542s`, `58.838s` (median `56.542s`, avg `57.266s`), same `3,659,074,945` steps, `31` GCs, high-water `138,068,154` cells, output SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, and `cell_size_bytes=8`. Previous refresh after the stale GC young-profile trim was `56.364s`, `55.941s`, `56.133s` (median `56.133s`, avg `56.146s`); treat the current refresh as the same mid-56s band with one noisy slow sample, not a new source-speed claim. Latest paired accepted 128M gate remains the free-slot allocation cleanup: candidate median `58.044s` and avg `58.040s` versus same-session baseline median `57.939s` and avg `58.084s` (`+0.18%` median, `-0.08%` avg). Treat that as neutral cleanup; the previous R5 sweep free-node row is the latest source-speed claim. Most recent fairer-memory 80Mi interval median is `68.975s` with high-water `88.15M` 8-byte cells and 49 GCs. Fresh C oracle full self-host is `47.85s`; older historical gates remain recorded below |
...
| full self-host compile | latest current-source 128M full self-host refresh after the stale per-head eval timing trim completed byte-identically in three samples: `56.419s`, `56.542s`, `58.838s` (median `56.542s`, avg `57.266s`), same `3,659,074,945` steps, `31` GCs, GC pauses `8.708s`, `8.747s`, `8.747s` (median `8.747s`, avg `8.734s`), high-water `138,068,154` cells, sink `661902`, `cell_size_bytes=8`, `cmp=0`, and SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`. Previous refresh after the stale GC young-profile trim was `56.364s`, `55.941s`, `56.133s` (median `56.133s`, avg `56.146s`), so this is a current-source refresh in the same mid-56s band with one noisy slow sample, not a new speed claim. The prior accepted free-slot allocation cleanup was byte-identical in a paired 3x full self-host A/B at the 128M window: baseline median `57.939s`/avg `58.084s` versus candidate median `58.044s`/avg `58.040s`, same heap shape and output; treat it as neutral cleanup (`+0.18%` median, `-0.08%` avg), not as a speed claim. The previous R5 sweep free-node fast path remains the accepted speed row. Fresh C oracle full self-host completes in `47.85s` with the same output SHA; tagged checkpoint remains `self-hosting-binary-match` at `673.8s`; use `timeout 900s` for full gates and profiling while structural/codegen probes can still regress above the smaller window |
