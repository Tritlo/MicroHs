...
| parity state | benchmark and smoke sinks match; Rust self-host compile produces byte-identical output. Current runtime uses unconditional 8-byte cells. The current full self-host row is the paired 128M cleanup gate: candidate avg `62.413s` versus committed packed-cell baseline avg `62.550s`, same `3,659,075,230` steps, `31` GCs, high-water `138,068,242` cells, matching output (`cmp_result=0`), and `cell_size_bytes=8`. Treat that as performance-neutral cleanup, not a new speed claim; prior best 128M no-env median remains `61.963s` from the interval probe and should be quoted with heap/RSS annotation. Current fairer-memory 80Mi interval median is `68.975s` with high-water `88.15M` 8-byte cells and 49 GCs. Fresh C oracle full self-host is `47.85s`; older historical gates remain recorded below |
