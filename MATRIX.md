...
| gate | status |
|---|---|
| Rust fmt/test/release bench build | latest current-source gates after deleting the rejected `moving-gc` scaffold: `cargo fmt --check`, `cargo check`, `cargo check --all-targets`, `cargo check --features gc-phase-profile`, `cargo check --features eval-phase-profile`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, broad source grep for stale moving-GC symbols, 100M/32M bounded self-host sanity, and `git diff --check` passed. Historical feature-gated moving-GC rows below remain as rejected/retired experiment records only |
