...
| item | state |
|---|---|
| branch | `microhs-rust`, currently synced with `origin/microhs-rust`; `split-runtime-modules` was fast-forward merged and remains as the source-organization side branch |
...
| parity state | benchmark and smoke sinks match; Rust self-host compile produces byte-identical output. Current runtime uses unconditional 8-byte cells. Latest current-source 128M full self-host refresh after the performance-neutral simplification pass is 3x byte-identical at `56.086s`, `55.855s`, `55.869s` (median `55.869s`, avg `55.937s`), `3,659,074,793` steps, `31` GCs, median GC pause `8.710s`, high-water `138,068,138` cells, output SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, and `cell_size_bytes=8`. The later R1 dirty-card work was feature-only, rejected, and reverted; default source remains in the same mid-56s band. Most recent fairer-memory 80Mi interval median is `68.975s` with high-water `88.15M` 8-byte cells and 49 GCs. Fresh C oracle full self-host is `47.85s`; older historical gates remain recorded below |
...
| full self-host compile | latest current-source 128M full self-host refresh after the stale WASI trace trim, old makefile removal, cold clippy cleanup, and LZMA decoder module move completed byte-identically in three samples: `56.086s`, `55.855s`, `55.869s` (median `55.869s`, avg `55.937s`), `3,659,074,793` steps, `31` GCs, GC pauses `8.710s`, `8.684s`, `8.754s` (median `8.710s`, avg `8.716s`), high-water `138,068,138` cells, sink `661902`, `cell_size_bytes=8`, `cmp=0`, and SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`. The later feature-gated R1 dirty-card probe was byte-identical but rejected/reverted. Fresh C oracle full self-host completes in `47.85s` with the same output SHA; tagged checkpoint remains `self-hosting-binary-match` at `673.8s`; use `timeout 900s` for full gates and profiling while structural/codegen probes can still regress above the smaller window |
...
| priority | work |
|---|---|
| 1 | Continue performance-neutral simplification, but only where it removes real surface area or stale profiling/runtime structure. Rev 5's main target is cfg-gating the runtime `profile: Option<EvalProfile>` machinery behind a cargo feature so default builds carry less profiling dataflow; do not spend more time shaving public API unless it deletes meaningful coupling |
