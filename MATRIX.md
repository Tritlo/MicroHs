...
| next moving prototype | a cheaper old-to-young source must avoid touching every hot app rewrite. Given S4, the rejected exact remembered-source probe, rejected coarse dirty cards, rejected branchless dirty cards, rejected periodic old compaction, and the accepted sparse/selected remap results, do not retry hot setter barriers/card marking or full-heap copy cleanup as standalone R1 work. Prefer allocation-site-specific old-update hooks that stay out of common app rewrites or a nursery/heap shape that reduces promotion and does not need per-rewrite old-to-young tracking; keep the nursery suffix/evacuation machinery as the correctness base |
| avoid | do not optimize the conservative old-space scan as if it were the final collector. It proves root/fixup correctness but becomes O(old) and loses the full gate even at the 128M speed window; sparse remapping was worth accepting only because it removed a separate whole-heap remap-table cost without adding hot-path overhead |
...
| 2 | Pivot to simplification/review. First target: delete the dead `moving-gc` scaffold (`moving-gc` feature, remap/evacuate modules, nursery collector fork, feature-only allocation/GC branches, and moving-GC tests) now that R1 went negative and PGO quantified the floor |
| 3 | After scaffold deletion, sweep rejected-probe residue and unused cfg gates, then tighten module docs and architecture docs for the reducer, 8-byte cell/cold side table, and mark-sweep GC |
