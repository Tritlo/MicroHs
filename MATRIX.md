...
| active working tree | default runtime behavior now includes the accepted packed 8-byte `Cell` arena (`default = ["packed-cell"]`), while the prior 16-byte cell path remains available with `--no-default-features`; the last committed runtime checkpoint before this probe is `d5efc37f`, including trusted internal WHNF resolution, trusted app-fun descent, and trusted hot reducer cell reads in the reducer. Current source organization is real Rust modules, not `include!`: `runtime.rs` declares the public facade and internal module tree; `runtime/host.rs`, `runtime/program.rs`, `runtime/program/bfile.rs`, `runtime/program/eval.rs`, and `runtime/program/runtime_dispatch.rs` are `mod` roots with scoped imports. The live fallback reducer is no longer named legacy: `eval/reduce.rs`, `eval/spine.rs`, `eval/persistent.rs`, and `eval/fallback.rs` hold the general loop, spine helpers, persistent reducer, and runtime-primitive fallback; eval-time primitive dispatch is `eval/dispatch.rs`. Runtime modules have one-line `//!` docs and internal items are scoped to `crate::runtime` where practical. Release builds now use `panic = "abort"`, `lto = "thin"`, and `codegen-units = 1`, so module layout should not perturb optimized hot-loop codegen. Native helpers live under `rust/microhs-runtime/tools/native/`; browser JS-FFI wasm harnesses live under `rust/microhs-runtime/tools/wasm/browser/`; the WASI self-host harness lives under `rust/microhs-runtime/tools/wasm/wasi/`; the generic `wasi-run.mjs` harness and legacy process-spawning `--c-mhseval` benchmark path have been removed while the in-process `--c-mhsbench` comparison remains. Current runtime also has committed feature-gated profiling counters, the GC-rooted read-only memory BFILE view, checked LZMA decoding, weak-pointer key/value GC semantics, and the self-host/PGo harnesses; `MATRIX.md` is the live ledger and local scratch notes are present |
...
| node layout | bench now prints representation sizes; current default release output is `node_size_bytes=16`, `cell_size_bytes=8`, `node_id_size_bytes=4`, `prim_size_bytes=4`. The authoritative arena is the packed default `Cell { word }`: 4-bit tag, u30 App fun/arg payloads, u30 Indir/Free/Cold payloads, 60-bit small `Int`/`ThreadId`, inline `Float32`, and cold-boxed wide scalars/payloads; the previous `Cell { word0, word1 }` 16-byte layout remains available with `--no-default-features`. `Node` stays the parse/debug facade for cold boxed payloads |
| parity state | benchmark and smoke sinks match; Rust self-host compile produces byte-identical output. Current packed-cell default was accepted by a paired full self-host A/B at the 128M window: 16-byte fallback median `63.580s` versus packed default median `62.750s`, same `3,659,075,040` steps, `31` GCs, high-water `138,068,222` cells, matching output (`cmp_result=0`), and `cell_size_bytes` `16 -> 8`. Prior best 128M no-env median remains `61.963s` from the interval probe, so quote the packed result as an accepted paired source win rather than a new absolute wall-clock best. Older default-release full gate was committed `d5efc37f` at 74.668s, byte-identical, 49.00M steps/s, 124 GCs, and 23.445s GC pause. Latest live-reducer split gate at `8933a6d1` completed byte-identically in 74.904s with 3,659,073,466 steps, 48.85M steps/s, 124 GCs, 24.815s GC pause, high-water 39,192,385 cells, and the same output SHA. Latest real-module plus thin-LTO gate at `76ff9d3e` completed byte-identically in 77.240s with 3,659,073,428 steps, 47.37M steps/s, 124 GCs, 25.367s GC pause, high-water 39,192,381 cells, and the same output SHA; the same real-module source just before LTO was byte-identical at 78.807s with 46.43M steps/s and 25.121s GC pause. Prior include-based source-organization reorg gate completed byte-identically in 78.677s with 3,659,074,736 steps, 46.51M steps/s, 124 GCs, 25.286s GC pause, high-water 39,192,398 cells, and the same output SHA; source movement only, not a new performance baseline. Fresh clean-source current refresh after the WASI harness commit completed byte-identically in 77.500s with 3,659,074,698 steps, 47.21M steps/s, 124 GCs, 24.858s GC pause, high-water 39,192,398 cells, and the same output SHA. Prior `c969e993` best was 76.369s / 47.91M steps/s / 23.662s GC; prior `3c04e5c1` best was 77.050s / 47.49M steps/s / 23.629s GC, with a noisy 83.241s repeat; prior clean `65c3be1d` archive best was 77.359s / 47.30M steps/s / 23.225s GC; previous `c62cef08` best was 77.387s / 47.28M steps/s / 23.256s GC. Same-session current refreshes before this were 77.918s / 46.96M steps/s / 23.424s GC, 78.093s / 46.86M steps/s / 23.616s GC after feature-gated app-shape diagnostics, 77.533s / 47.19M steps/s / 23.322s GC at committed `65c3be1d`, 78.117s / 46.84M steps/s / 23.416s GC after uncommitted resolved app-shape diagnostics, and 77.592s / 47.16M steps/s / 23.314s GC after isolated WASI cfg/trace work, the first normal-release gate at 78.857s / 46.40M steps/s / 23.824s GC, and an out-of-tree `RUSTFLAGS=-Cpanic=abort` probe at 79.686s / 45.92M steps/s / 23.610s GC; prior `a82740a8` default gate repeated at 80.309s / 80.198s, 45.56M / 45.63M steps/s, and about 23.8s GC pause; prior runtime checkpoint `4cc9879a` repeated at 80.584s / 81.336s, 45.41M / 44.99M steps/s, and about 23.89s GC pause; prior all-time best was the eval.c permanent compound cache run at 80.927s, byte-identical, 45.21M steps/s, and 23.793s GC pause; fresh C oracle full self-host is 47.85s |

## Verification Gates

| gate | status |
|---|---|
| Rust fmt/test/release bench build | passed after the S3 authoritative cell arena, S1 single-stack hot path, S2 GC + S1 app-only spine/head-classification worktree, rejected strict-tail/segmented-stack reverts, S3 parse-time primitive interning, S3 compact representation work, direct runtime-primitive strict dispatch, S1 stack argument binding, S1 compact stack-entry split, S1 trusted stack argument loads/static fallback names, S1 app-only eval stack, S0 layer/update profiling, feature-gated phase/profile-overhead profiling, strict Int immediate args, unchecked stack arg loads, reusable GC mark-work stack, feature-gated GC phase profiling, rejected bitmap allocator restore, S1 app-continuation descent, S1 direct app-result descent, S1 dedicated app allocation, stack scalar `SET*` redex overwrites, S1 fixed-arity `CHKARG` pop/take rewrites, direct known-app/free-cell writes, S1 strict `Int` redex-owning frames, S1 WHNF redex-owning frames, raw cell tag/trusted free-list accessors, parse-time small-int interning retest, F10 iterative serializer, F17 cold bytes views, F14 shared ForeignPtr finalizer records, F12 catch masking-state restoration, per-head/per-site eval profiling, bench representation-size output, S1 unified stack `ret`/`top` continuation, S2 parse-label non-root GC treatment, S1 allocator free-head invariant tightening, S1 identity-alias `GOIND` continuation, S1 one-word app-fun spine descent, S2 mark-time app-edge canonicalization, S1 trusted stack redex/update access, eval.c permanent compound caches, rejected low-to-high free-list reuse restore, rejected FFI/JS arity-prefix materialization restore, rejected eval.c `GOAP2` direct-push restore, app-allocation split profiling instrumentation, GCRED opportunity profiling instrumentation, F10 BFILE `IO.print`/`IO.serialize` parity, S4 young-region viability profiling, F6/F10 `IO.deserialize` parity, weak-pointer GC semantics, the app-allocation profiling-bookkeeping cold split, inline primitive cell decoding, release `panic=abort` with a checked LZMA decoder, feature-gated stack-head arity/continuation/app-shape profiling, trusted non-profile WHNF resolver, trusted app-fun descent, trusted hot reducer cell reads following eval.c's internal-heap trust rule, real runtime modules, and the live reducer split/rename; latest `cargo fmt --check`, `cargo check`, `cargo test --lib` 41/41, `cargo check --features eval-phase-profile`, `cargo check --features gc-phase-profile`, release wasm checks for `wasm32-unknown-unknown` and `wasm32-wasip1`, release bench build, 10M/100M bounded gates, current `eval-phase-profile` 10M refresh, stack-head arity-profile refresh, stack-continuation transition profile, app-allocation site-shape profile, resolved app-allocation site-shape profile, trusted-WHNF 100M A/B/full gates, trusted-app-fun 8x 100M A/B/full gate, trusted-hot-cell 8x 100M A/B/full gate, latest same-session 900s full self-host refresh, rejected dense free-slot stack allocator probe, rejected direct B/B superinstruction full gate, rejected raw app-stack push bounded probe, rejected batched app-allocation accounting full gate, rejected strict-Int indirection immediate full gate, restored intrusive-free-list 100M sanity, SHA/cmp checks, and `git diff --check` passed |
...
| full self-host compile | current packed-cell default paired full gate at the 128M window is byte-identical with median `62.750s` (`62.750s`, `63.183s`, `62.687s`), `3,659,075,040` steps, `31` GCs, avg `9.998s` GC pause, high-water `138,068,222` cells, sink `661902`, and output cmp success; paired 16-byte fallback median was `63.580s`, so the accepted source delta is `-1.31%` average while halving `cell_size_bytes`. Prior best 128M no-env median remains `61.963s` from the interval probe, so keep the heap/RSS annotation when quoting it. Older 32M default-release gate is committed `d5efc37f`, completing byte-identically in 74.668s with 3,659,074,850 steps, 49.00M steps/s, 124 GCs, 23.445s GC pause, high-water 39,192,414 cells, and SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`; latest live-reducer split checkpoint `8933a6d1` completed byte-identically in 74.904s with 3,659,073,466 steps, 48.85M steps/s, 124 GCs, 24.815s GC pause, high-water 39,192,385 cells, and the same output SHA. Fresh clean-source current refresh after `c5f8ea1f` completed byte-identically in 77.500s with 3,659,074,698 steps, 47.21M steps/s, 124 GCs, 24.858s GC pause, high-water 39,192,398 cells, and the same output SHA. Prior `c969e993` trusted app-fun gate was byte-identical at 76.369s with 3,659,074,983 steps, 47.91M steps/s, 124 GCs, 23.662s GC pause, and high-water 39,192,428 cells; prior `3c04e5c1` trusted-WHNF gate was byte-identical at 77.050s with 3,659,075,002 steps, 47.49M steps/s, 124 GCs, 23.629s GC pause, and high-water 39,192,430 cells. Prior clean `65c3be1d` archive rebuild completed byte-identically in 77.359s with 3,659,074,888 steps, 47.30M steps/s, 124 GCs, 23.225s GC pause, high-water 39,192,418 cells, and the same output SHA; previous best `c62cef08` completed byte-identically in 77.387s with 3,659,074,774 steps, 47.28M steps/s, 124 GCs, 23.256s GC pause, high-water 39,192,406 cells, and the same output SHA. Best measured 32M execution mode was the `15f76b3a` self-host-trained PGO build, completing byte-identically in 72.387s with 3,659,074,736 steps, 50.55M steps/s, 125 GCs, 25.933s GC pause, high-water 38,820,694 cells, with a 72.160s / 50.71M steps/s / 25.906s GC repeat; fresh C oracle full self-host completes in 47.85s with the same output SHA; tagged checkpoint remains `self-hosting-binary-match` at 673.8s; use `timeout 900s` for full gates and profiling while structural/codegen probes can still regress above the smaller window |
...
| GC/F5 | safe-point mark/sweep plus S2 allocation-pressure trigger, intrusive free list, mark-time indirection compression, reusable mark-work stack, parse-label non-root treatment, optional `gc-phase-profile` mark/sweep timings, C-style sweep-time ForeignPtr finalizers, GCRED opportunity counters, S4 young-region viability counters, and eval.c-style weak key/value clearing are implemented; full gates pass, but scheduler-backed weak finalizer spawning, actual minor collection, and full GCRED semantics are still pending |
...
| priority | work |
|---|---|
| 1 | Attack the measured post-raw-tag/free-list S1 buckets in order: app allocation, inner descent, arg reads, and app updates. The stack-head arity profile says the hot B/C/S heads are mostly over-applied under long outer spines, so prefer a vertical eval.c-shaped change that consumes the outer context or removes work from those buckets over local codegen, scalar/tag probes, or exact-shape superinstructions |
| 2 | Push from the current 74.668s default-release best closer to C; use full self-host as the arbiter because the accepted strict-`Int` slice won full despite a slightly slower 100M bounded slice, the WHNF slice won full despite a noisier feature-build 10M control, the raw-tag/free-list slice won the full gate while the 10M feature-build control stayed noisy, F14's 100M slice was slower before the final cold fix while the full gate became the new best, the guarded-`GOIND` probe only became useful after the broader ready-return layer moved into the stack loop, the parse-label root fix won full despite a noisy/slower bounded mutator slice, the free-head invariant slice won both repeated 100M and full gates, the identity-alias `GOIND` slice won full only slightly despite large exit-counter reductions, one-word app-fun descent won the full gate despite a slower/noisy 100M bounded slice, app-edge canonicalization won full by changing heap shape despite a locally higher 100M GC pause, trusted stack redex/update access won full by removing a real Rust-only hot-path error layer, permanent compound caches won full by copying eval.c's permanent helper-prefix shape, the app-allocation bookkeeping, inline decoder, and release panic-abort slices won by recovering source-level/default-build parts of Rust's codegen debt, and the trusted-WHNF/app-fun/hot-cell series won by trusting internal heap invariants the way eval.c does |
...
| 4 | For non-self-host parity/perf, keep `ffi-mem-chain`, `bfile-read-chain`, and `utf8-bfile-read-chain` as canaries. The committed read-only memory BFILE view proves an isolated host-buffer change can help and pass self-host, but the main frontier remains F2/S1 app allocation, descent, arg reads, and app updates |
