...
| priority | work |
|---|---|
| 1 | Performance-neutral simplification is active now that NOTES rev 4 is closed out: R2 partial-line was rejected for alloc-path cost, 8B/u30 cells are accepted/default, and the old mutator-probe queue is no longer the immediate path. Prefer trims that remove stale representation axes, unused public API, duplicated cold paths, or misleading scaffolding without changing the self-host heap shape |
| 2 | Gate sizeable simplifications with the current protocol: 8-run alternating 100M/32M A/B for source neutrality, and paired 3x full self-host at the 128M speed window when the change touches evaluator, allocator, GC roots, dispatch layout, or large runtime code shape. Cold/API-only cleanups still need `cargo fmt --check`, `cargo check`, feature checks, `cargo test --lib`, release bench build, wasm/browser build, and WASI build |
| 3 | Keep R1 copying nursery as the next structural performance lead after the simplification pass. It is the only remaining direction that can improve both sweep cost and mutator descent locality, but it breaks stable `NodeId`, so start with a root/handle/fixup audit rather than a blind collector prototype |
