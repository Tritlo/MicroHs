...
| parity state | benchmark and smoke sinks match; Rust self-host compile produces byte-identical output. Current runtime uses unconditional 8-byte cells. Latest paired 128M full self-host gate is the accepted free-slot allocation cleanup: candidate median `58.044s` and avg `58.040s` versus same-session baseline median `57.939s` and avg `58.084s` (`+0.18%` median, `-0.08%` avg), same `3,659,074,945` steps, `31` GCs, high-water `138,068,212` cells, matching output (`cmp_all=1`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`), and `cell_size_bytes=8`. Treat this as neutral cleanup; the previous R5 sweep free-node row is the latest source-speed claim. Most recent fairer-memory 80Mi interval median is `68.975s` with high-water `88.15M` 8-byte cells and 49 GCs. Fresh C oracle full self-host is `47.85s`; older historical gates remain recorded below |
...
| full self-host compile | current free-slot allocation cleanup was byte-identical in a paired 3x full self-host A/B at the 128M window: baseline samples `58.445s`, `57.868s`, `57.939s` (median `57.939s`, avg `58.084s`) versus candidate samples `57.937s`, `58.141s`, `58.044s` (median `58.044s`, avg `58.040s`), same `3,659,074,945` steps, `31` GCs, high-water `138,068,212` cells, sink `661902`, `cell_size_bytes=8`, `cmp_all=1`, and SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`. Treat this as neutral cleanup (`+0.18%` median, `-0.08%` avg), not a new source-speed claim; the previous R5 sweep free-node fast path remains the accepted speed row. Fresh C oracle full self-host completes in `47.85s` with the same output SHA; tagged checkpoint remains `self-hosting-binary-match` at `673.8s`; use `timeout 900s` for full gates and profiling while structural/codegen probes can still regress above the smaller window |
...
|---|---|
| first artifact | build a shared mutable `NodeId` visitor/remapper and run it in an identity-remap/assert mode before writing a moving collector. It should cover all holders above and can be checked against the existing mark root enumeration |
| first moving prototype | prefer a full-heap or nursery-only evacuation behind a feature after the remapper exists. Preserve guest pointer integers by remapping `node_pointers` slots and rebuilding `node_pointer_slots` |
...
| priority | work |
|---|---|
| 1 | Start the R1 prep with a shared mutable `NodeId` visitor/remapper in identity mode. It should cover heap cells, cold payloads, `Program` side roots, evaluator stacks/spines/frames, weak/finalizer queues, BFILE read-only memory view bases, and `node_pointers`/`node_pointer_slots`. This is the correctness gate before any copying nursery prototype |
| 2 | Keep performance-neutral simplification opportunistic, but do not spend more time shaving public API unless it removes real surface area. R2 partial-line was rejected for alloc-path cost, 8B/u30 cells are accepted/default, and the old mutator-probe queue is no longer the immediate path |
| 3 | Gate any sizeable runtime simplification or R1 scaffold with the current protocol: 8-run alternating 100M/32M A/B for source neutrality, and paired 3x full self-host at the 128M speed window when the change touches evaluator, allocator, GC roots, dispatch layout, or large runtime code shape. Cold/API-only cleanups still need `cargo fmt --check`, `cargo check`, feature checks, `cargo test --lib`, release bench build, wasm/browser build, and WASI build |
