...
| 2 | Continue R1 from the proven `moving-gc` collector bridge by narrowing evacuation to a nursery/recent-allocation range. Preserve guest pointer integers by keeping `node_pointers` targets marked/remapped and rebuilding `node_pointer_slots`; do not add per-write barrier checks until a remembered-set design is unavoidable |
