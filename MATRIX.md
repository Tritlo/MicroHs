...
| local commits ahead after this snapshot commit | 96 |
| runtime code baseline | F2 Int continuation-stack checkpoint |
...
Last fully verified state: F2 Int continuation-stack checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed before F2 Int continuation-stack commit; 34 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed before F2 Int continuation-stack commit |
| `cargo check -p microhs-runtime --bins --quiet` | passed before F2 Int continuation-stack commit |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before F2 Int continuation-stack commit |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before F2 Int continuation-stack commit |
...
| `cargo fmt --all --check` | passed before F2 Int continuation-stack commit |
| `git diff --check` | passed before F2 Int continuation-stack commit |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed before F2 Int continuation-stack commit |
...
| F2 Int continuation-stack probe | passed before checkpoint commit; `eval_int` now forces nested Int unary/binary primops through an explicit inline-one-frame continuation stack instead of recursively calling `reduce_node_whnf`; `arith-chain:200` improves to Rust `36,122` ns/iter vs C `108,853` with matching sink, but self-host `--help` regresses to `80,617,930` ns/iter, so this is a correctness/structure checkpoint rather than a compiler-speed win |
...
Latest tracked runtime behavior change is the first F2 marker-continuation slice: `eval_int` handles nested Int unary/binary primops with an explicit continuation stack instead of native recursive reducer calls.
...
| `--help` main smoke | 9,934,460 ns/iter; sink `661902` per iter | 80,617,930 ns/iter; sink `913271` per iter | both run and print identical usage text; not a full compile; refreshed with `--warmup-iters 1 --iters 3` after the F2 Int continuation-stack slice; this fixes Int strict-chain shape and speeds arithmetic, but currently regresses the compiler proxy pending extension/optimization of the marker machine |
