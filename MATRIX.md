...
| local commits ahead after this snapshot commit | 107 |
| runtime code baseline | F2 guarded Int marker-stack slice |
...
Last fully verified state: F2 guarded Int marker-stack checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed after F2 guarded Int marker-stack slice; 34 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed after F2 guarded Int marker-stack slice |
| `cargo check -p microhs-runtime --bins --quiet` | passed after F2 guarded Int marker-stack slice |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 guarded Int marker-stack slice |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 guarded Int marker-stack slice |
...
| `cargo fmt --all --check` | passed after F2 guarded Int marker-stack slice |
| `git diff --check` | passed after F2 guarded Int marker-stack slice |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed after F2 guarded Int marker-stack slice |
...
| self-host Rust main smoke | not yet passing; no-shim run no longer falsely invokes `cpphs`; latest guarded Int-marker full compile run exited `101` before the 300s cap, printed the missing `mhs.conf` warning to `/tmp/mhs-selfhost-rust-300s-20260703-035126.out`, and failed with a layout parse error at `src/MicroHs/Main.hs:60`; the same command under C succeeds |
...
| `--help` main smoke | 10,956,345 ns/iter; sink `661902` per iter | 63,918,846 ns/iter; sink `661902` per iter | both run and print identical usage text; not a full compile; refreshed with `--warmup-iters 1 --iters 3` after the F2 guarded Int marker-stack slice; this slice is an arithmetic/common-case win, while the compiler proxy is noisy/neutral and still about 5-6x C on this busy machine |
| self-host compiler smoke | 61,920,977,046 ns/iter; output `/tmp/mhs-selfhost-c-current-source-check.comb` is a 647 KiB comb | latest guarded Int-marker run exited `101` before the 300s cap with a layout parse error at `src/MicroHs/Main.hs:60`; no output comb existed | not at parity; all-Int marker probes reached earlier parser failures, so comparison/ordering markers remain on the old helper path for now |
...
`target/release/mhs-rust-bench --input /tmp/mhs-selfhost.comb --mode main --warmup-iters 0 --iters 1 --profile --profile-top 20 -- ./bin/mhs --help > /tmp/mhs-selfhost-int-marker-guarded-profile.out`
...
| measure | count | implication |
|---|---:|---|
| profiled iteration time | 107.280 ms | instrumentation overhead is separate from normal timing; raw time is noisy |
| reductions | 297,417 | F2 evaluator-owned spine loop and guarded Int marker stack preserve the current work count; speedup comes from loop mechanics, not changed semantics |
...
| Use marker continuations for strict primitive forcing | Binary/unary int, int64, float, double, and bytes primops push nodes such as `T_BININT2`/`T_BININT1` and finish in `ret`, with fast paths when operands are already boxed values | Rust now has cheap fast paths for already-WHNF coercion arguments plus explicit continuation stacks for nested Int, Int64 including mixed shifts, Float64, Float32, binary ByteString marker forcing, and the strict WHNF arms `seq`, `IO.strict`, and `isInt`; the main WHNF driver now owns a guarded Int-result marker stack for non-`KnownPrim` heads, while Int comparison/ordering, Int64/float/bytes/generic strict helpers still need the same single-loop treatment | partial |
...
| self-hosting parity | C runtime can execute the compiler comb and produce a new compiler comb | Rust main-mode harness exists and lazy file IO now matches the C CPP scan smoke; latest full C comparison succeeds in `61,920,977,046` ns/iter with sink `661902`, while latest guarded Rust run exits `101` before the 300s cap with a layout parse error at `src/MicroHs/Main.hs:60`; inline spine work removed almost all `--help` heap-spine spills, generalized combinator app reuse cuts self-host `--help` node growth by ~27%, direct known-head/helper dispatch plus C-style `Y` sharing keep the short self-host proxy in the ~60-85 ms band, small-int caching trims final profile nodes to 615,854, strict WHNF coercion fast paths improved the proxy before noisy reruns, F2 numeric/ByteString/WHNF/rnf slices remove several native-recursion paths, the F2 evaluator-owned WHNF spine-loop slice drops the compiler-shaped proxy to Rust `61,450,144` ns/iter vs C `12,341,422`, and the guarded Int-result marker slice improves arithmetic/common canaries while leaving self-host `--help` noisy/neutral at Rust `63,918,846` ns/iter vs C `10,956,345`; full self-host parity still needs the layout parse failure isolated plus Int comparison/ordering, Int64/float/bytes/generic strict primops, and helper evaluators merged into the marker-machine loop |
...
| performance phase | F2 single-stack reducer work active after twenty-eight performance checkpoints plus one benchmark parity checkpoint plus static, dynamic, Rust/LLVM, resolve/shortcut/allocation profile, and `eval.c` lesson checkpoints; numeric and bytes benchmark matrix rows now all sink-match C, the strict numeric/binary-ByteString coercion paths have explicit continuation stacks for Int, Int64 including mixed shifts, Float64, Float32, and binary ByteString marker ops, the normal WHNF driver owns the hot spine/rewrite loop, and guarded Int-result marker forcing has moved into that driver for non-`KnownPrim` heads; rows are split into common-case and rare/specialized; next common-operation targets are the self-host layout failure from all-Int marker forcing, Int comparison/ordering markers, Int64/float/bytes/generic strict primops and helper-return dispatch on the same marker stack, plus remaining repeated resolve/node-classification and app allocation/update mechanics | active |
