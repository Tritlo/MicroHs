...
| local commits ahead after this snapshot commit | 98 |
| runtime code baseline | F2 Int/Int64/Float64/Float32 continuation-stack checkpoint |
...
Last fully verified state: F2 Int/Int64/Float64/Float32 continuation-stack checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed before F2 Int/Int64/Float64/Float32 continuation-stack commit; 34 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed before F2 Int/Int64/Float64/Float32 continuation-stack commit |
| `cargo check -p microhs-runtime --bins --quiet` | passed before F2 Int/Int64/Float64/Float32 continuation-stack commit |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before F2 Int/Int64/Float64/Float32 continuation-stack commit |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before F2 Int/Int64/Float64/Float32 continuation-stack commit |
...
| `cargo fmt --all --check` | passed before F2 Int/Int64/Float64/Float32 continuation-stack commit |
| `git diff --check` | passed before F2 Int/Int64/Float64/Float32 continuation-stack commit |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed before F2 Int/Int64/Float64/Float32 continuation-stack commit |
...
| F2 numeric continuation-stack probe | passed before checkpoint commit; `eval_int`, non-shift `eval_int64`, `eval_float64`, and `eval_float32` now force nested unary/binary primops through explicit inline-one-frame continuation stacks instead of recursively calling `reduce_node_whnf`; `arith-chain:200` is Rust `36,122` ns/iter vs C `108,853`, `int64-chain:200` is Rust `35,442` ns/iter vs C `112,619`, `float64-chain:200` is Rust `40,276` ns/iter vs C `120,763`, `float32-chain:200` is Rust `48,362` ns/iter vs C `111,033`, and self-host `--help` is `68,235,892` ns/iter after the float slice; still a structural checkpoint rather than a full compiler-speed win |
...
Latest tracked runtime behavior change is the current F2 numeric marker-continuation slice: `eval_int`, non-shift `eval_int64`, `eval_float64`, and `eval_float32` handle nested unary/binary primops with explicit continuation stacks instead of native recursive reducer calls.
...
| `--help` main smoke | 9,967,646 ns/iter; sink `661902` per iter | 68,235,892 ns/iter; sink `913271` per iter | both run and print identical usage text; not a full compile; refreshed with `--warmup-iters 1 --iters 3` after the F2 Float64/Float32 continuation-stack slice; the broader numeric marker-machine work has mostly recovered the strict-WHNF proxy win, but full self-host still needs bytes and reducer-loop work |
...
| performance phase | structural F2 work active after twenty-four performance checkpoints plus one benchmark parity checkpoint plus static, dynamic, Rust/LLVM, resolve/shortcut/allocation profile, and `eval.c` lesson checkpoints; numeric benchmark matrix rows now all sink-match C and the strict numeric coercion paths have explicit continuation stacks for Int, non-shift Int64, Float64, and Float32; rows are split into common-case and rare/specialized; common-operation targets have shifted from narrow IO/FFI/data-constructor probes to C-shaped reducer work: explicit marker continuations, stack-like spine traversal, repeated resolve/node-classification traffic, helper-return dispatch, and app allocation/update mechanics | active |
