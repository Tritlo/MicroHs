...
| next moving prototype | a cheaper old-to-young source must avoid a branch in every hot app rewrite. Given S4, the rejected exact remembered-source probe, and the rejected coarse dirty-card probe, do not retry hot setter barriers as standalone R1 work. Prefer allocation-site-specific old-update hooks that stay out of common app rewrites, periodic old-space compaction, or a nursery design that does not need per-rewrite old-to-young tracking; keep the nursery suffix/evacuation machinery as the correctness base |
