...
| next moving prototype | narrow the feature path from full-heap evacuation to nursery-only movement: keep old-space mark-sweep or no-op for now, evacuate only recent allocation ranges, and avoid per-write barrier checks until a remembered-set design is required. Full-heap moving is a correctness bridge and profiling probe, not the target collector |
