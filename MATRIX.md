...
| parity state | benchmark and smoke sinks match; Rust self-host compile produces byte-identical output. Current runtime uses unconditional 8-byte cells. Latest paired 128M full self-host gate is the accepted R5 sweep free-node fast path: candidate median `58.503s` and avg `58.474s` versus same-session baseline median `59.319s` and avg `59.392s`, same `3,659,074,812` steps, `31` GCs, high-water `138,068,198` cells, matching output (`cmp_all=1`, SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`), and `cell_size_bytes=8`. Most recent fairer-memory 80Mi interval median is `68.975s` with high-water `88.15M` 8-byte cells and 49 GCs. Fresh C oracle full self-host is `47.85s`; older historical gates remain recorded below |
...
| full self-host compile | current R5 sweep free-node fast path was byte-identical in a paired 3x full self-host A/B at the 128M window: baseline samples `59.742s`, `59.116s`, `59.319s` (median `59.319s`, avg `59.392s`) versus candidate samples `58.503s`, `58.698s`, `58.222s` (median `58.503s`, avg `58.474s`), same `3,659,074,812` steps, `31` GCs, high-water `138,068,198` cells, sink `661902`, `cell_size_bytes=8`, `cmp_all=1`, and SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`. GC pause avg fell from `9.755s` to `9.000s`. Fresh C oracle full self-host completes in `47.85s` with the same output SHA; tagged checkpoint remains `self-hosting-binary-match` at `673.8s`; use `timeout 900s` for full gates and profiling while structural/codegen probes can still regress above the smaller window |
