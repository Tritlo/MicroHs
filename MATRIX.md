...
| F2 bridge lesson | Exposing the existing `IntFrame` helper stack through a `StepAction` request is not enough; it changed `arith-chain:200` from one outer WHNF step to 200 driver-counted steps and regressed the common arithmetic row despite matching sinks. The next F2 attempt needs a real unified evaluator loop, not a bridge that bounces through the current `StepResult` boundary. |
