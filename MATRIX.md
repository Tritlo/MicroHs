...
| local commits ahead after this snapshot commit | 147 |
| runtime code baseline | F2 generic conversion marker slice |
...
Last fully verified state: F2 generic conversion marker checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed after F2 generic conversion marker slice; 34 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed after F2 generic conversion marker slice |
| `cargo check -p microhs-runtime --bins --quiet` | passed after F2 generic conversion marker slice |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 generic conversion marker slice |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 generic conversion marker slice |
...
| `cargo fmt --all --check` | passed after F2 generic conversion marker slice |
| `git diff --check` | passed after F2 generic conversion marker slice |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed after F2 generic conversion marker slice |
...
Latest tracked runtime behavior change is the current F2 generic conversion marker slice: normal WHNF reduction now handles unary numeric/bit conversions through the same ordered `EvalFrameStack` as WHNF frames and strict-result markers. Earlier F2 slices remain in place: the evaluator-owned WHNF spine loop, `eval_int`, `eval_int64` including mixed Int64 shifts, `eval_float64`, `eval_float32`, binary ByteString marker ops, the strict WHNF arms `seq`, `IO.strict`, and `isInt`, and `rnf` have explicit stack/continuation paths instead of unbounded native recursion for nested chains. Latest benchmark harness change makes Rust main-mode sinks C-compatible by using the input-byte sink instead of graph-size-dependent output.
...
| `--help` main smoke | 10,543,728 ns/iter; sink `661902` per iter | 69,279,672 ns/iter; sink `661902` per iter | both run and print identical usage text; not a full compile; refreshed with `--warmup-iters 1 --iters 3 --c-mhsbench-mode main` after the F2 generic conversion marker slice; this particular run is noisy/slower than the previous F2 ordered-marker checkpoint, and profile counts show the conversion fold did not change the self-host proxy work mix |
...
| measure | count | implication |
|---|---:|---|
| profiled iteration time | 98.494 ms | instrumentation overhead is separate from normal timing; raw time is noisy |
