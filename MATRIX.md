...
| local commits ahead after this snapshot commit | 79 |
| runtime code baseline | catchable arithmetic RTS exceptions checkpoint |
...
Last fully verified state: catchable arithmetic RTS exceptions checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed before catchable arithmetic RTS exceptions commit; 31 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed before catchable arithmetic RTS exceptions commit |
| `cargo check -p microhs-runtime --bins --quiet` | passed before catchable arithmetic RTS exceptions commit |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before catchable arithmetic RTS exceptions commit |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before catchable arithmetic RTS exceptions commit |
...
| `cargo fmt --all --check` | passed before catchable arithmetic RTS exceptions commit |
| `git diff --check` | passed before catchable arithmetic RTS exceptions commit |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed before catchable arithmetic RTS exceptions commit |
...
Latest tracked runtime behavior change is catchable arithmetic RTS exceptions; Int/Int64 primitive divide-by-zero and signed overflow now raise Haskell exception nodes 4 and 7 under `catch`, matching the C runtime's RTS exception ABI.
