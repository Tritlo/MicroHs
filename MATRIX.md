...
| current runtime checkpoint | `3dc80e7d Borrow comb parser tokens` |
...
| gate | current status |
|---|---|
| Rust format/check/test/build | passed for `3dc80e7d`: `cargo fmt --all --check`, `cargo check -p microhs-runtime --bins --quiet`, `cargo test -p microhs-runtime --quiet`, release bin build, wasm lib build |
| `git diff --check` | passed before `3dc80e7d` |
...
argument resolves and borrowing parser token text. They are same-machine
...
| self-host `--help` proxy | 47,983,803 | 11,536,508 | 4.16 | sink `1,985,706` both sides |
...
profiled resolve calls roughly in half. Borrowing parser tokens moves it again
to about 48.0 ms by cutting self-host parse from about 12.8 ms to 9.2 ms per
iteration. The remaining gap is still mostly evaluator throughput.
...
| priority | work |
|---|---|
| 1 | treat 32-byte nodes plus lazy spine args plus borrowed parser tokens as the current performance baseline |
