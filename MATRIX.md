...
| local commits ahead after this snapshot commit | 108 |
| runtime code baseline | F2 guarded strict-result marker-stack slice |
...
Last fully verified state: F2 guarded strict-result marker-stack checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed after F2 guarded strict-result marker-stack slice; 34 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed after F2 guarded strict-result marker-stack slice |
| `cargo check -p microhs-runtime --bins --quiet` | passed after F2 guarded strict-result marker-stack slice |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 guarded strict-result marker-stack slice |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 guarded strict-result marker-stack slice |
...
| `cargo fmt --all --check` | passed after F2 guarded strict-result marker-stack slice |
| `git diff --check` | passed after F2 guarded strict-result marker-stack slice |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed after F2 guarded strict-result marker-stack slice |
...
| self-host Rust main smoke | not yet passing; no-shim run no longer falsely invokes `cpphs`; latest strict-result marker full compile run hit `timeout 300s` with exit `124`, printed only the missing `mhs.conf` warning to `/tmp/mhs-selfhost-rust-300s-20260703-041331.out`, left empty stderr, and produced no output comb; a requested 1200s follow-up is running at `/tmp/mhs-selfhost-rust-1200s-20260703-041847.*` |
...
Latest tracked runtime behavior change is the current F2 evaluator-owned WHNF spine-loop slice: `reduce_whnf_from` now keeps a reusable spine stack in the driver and performs hot WHNF reductions through direct redex-root mutation instead of routing every reduction through `StepResult`. Earlier F2 slices remain in place: `eval_int`, `eval_int64` including mixed Int64 shifts, `eval_float64`, `eval_float32`, binary ByteString marker ops, the strict WHNF arms `seq`, `IO.strict`, and `isInt`, and `rnf` have explicit stack/continuation paths instead of unbounded native recursion for nested chains. Latest benchmark harness change makes Rust main-mode sinks C-compatible by using the input-byte sink instead of graph-size-dependent output.
...
| `arith-chain:200` | 35,872 | 125,945 | 0.28 | yes |
| `int64-chain:200` | 38,175 | 108,656 | 0.35 | yes |
| `float64-chain:200` | 40,276 | 120,763 | 0.33 | yes |
| `float32-chain:200` | 48,362 | 111,033 | 0.44 | yes |
| `bytes-chain:200` | 58,048 | 178,207 | 0.33 | yes |
...
| `io-control-chain:200` | 59,537 | 142,261 | 0.42 | yes |
...
| `--help` main smoke | 12,363,403 ns/iter; sink `661902` per iter | 67,292,984 ns/iter; sink `661902` per iter | both run and print identical usage text; not a full compile; refreshed with `--warmup-iters 1 --iters 3` after the F2 guarded strict-result marker-stack slice; this slice is a numeric/bytes common-case win, while the compiler proxy is noisy/neutral and still about 5-6x C on this busy machine |
| self-host compiler smoke | 61,920,977,046 ns/iter; output `/tmp/mhs-selfhost-c-current-source-check.comb` is a 647 KiB comb | latest strict-result marker run hit `timeout 300s` with exit `124`; `/tmp/mhs-selfhost-rust-300s-20260703-041331.out` only has the missing `mhs.conf` warning, stderr is empty, and no output comb existed; requested 1200s follow-up is running at `/tmp/mhs-selfhost-rust-1200s-20260703-041847.*` | not at parity; gating markers out of WHNF-frame subloops restores timeout behavior rather than parser/layout failure |
...
`target/release/mhs-rust-bench --input /tmp/mhs-selfhost.comb --mode main --warmup-iters 0 --iters 1 --profile --profile-top 20 -- ./bin/mhs --help > /tmp/mhs-selfhost-strict-result-marker-profile.out`
...
| measure | count | implication |
|---|---:|---|
| profiled iteration time | 115.677 ms | instrumentation overhead is separate from normal timing; raw time is noisy |
| reductions | 297,417 | F2 evaluator-owned spine loop and guarded strict-result marker stacks preserve the current work count; targeted speedups come from loop mechanics, not changed semantics |
| step attempts | 300,279 | strict WHNF fast paths removed the earlier extra attempts; most work is real reduction |
...
| profiled resolve calls | 5,064,872 | resolve/node-classification traffic is still much larger than reduction count, but the selected loop removes part of the normal-run overhead around it |
| followed indirections | 102,179 | only about 2.0% of profiled resolve calls follow an indirection |
...
| Walk application spines directly on the evaluator stack | `evali` follows `T_AP` nodes with `PUSH(n)` until the head tag is known | The WHNF driver now owns a reusable `EvalSpine` and dispatches hot reductions from that stack; generic primops/FFI still materialize head-order args, and strict helper evaluators still have their own driver loops | partial |
| Rewrite the consumed redex cell and continue | `GOIND`, `GOAP`, and `GOAP2` mutate the current node/root app and jump back to `top`/`ap` | The selected F2 loop now mutates the consumed redex root directly for hot WHNF reductions and preserves root indirection for zero-arity no-spine reductions; full C shape still needs strict markers and helper evaluators merged into the same loop | partial |
...
| Use marker continuations for strict primitive forcing | Binary/unary int, int64, float, double, and bytes primops push nodes such as `T_BININT2`/`T_BININT1` and finish in `ret`, with fast paths when operands are already boxed values | Rust now has cheap fast paths for already-WHNF coercion arguments plus explicit continuation stacks for nested Int, Int64 including mixed shifts, Float64, Float32, binary ByteString marker forcing, and the strict WHNF arms `seq`, `IO.strict`, and `isInt`; the normal WHNF driver now owns guarded result-producing marker stacks for Int, Int64, Float64, Float32, and ByteString append, while comparison/ordering, mixed Int64 shifts, helper WHNF frames, and generic strict helpers still need one ordered marker stack | partial |
...
| F2 bridge lesson | Exposing the existing `IntFrame` helper stack through a `StepAction` request was not enough; it changed `arith-chain:200` from one outer WHNF step to 200 driver-counted steps and regressed the common arithmetic row despite matching sinks. The selected path instead keeps marker ownership inside `reduce_whnf_from`, guards the numeric-name probe behind `known.is_none()`, and disables these marker stacks inside `seq`/`IO.strict`/`isint` helper-frame subloops; those subloops need one ordered marker stack before branch-producing comparisons and mixed-result operations move over safely. |
...
| self-hosting parity | C runtime can execute the compiler comb and produce a new compiler comb | Rust main-mode harness exists and lazy file IO now matches the C CPP scan smoke; latest full C comparison succeeds in `61,920,977,046` ns/iter with sink `661902`; a guarded Rust run with markers disabled in WHNF-frame subloops restored the previous 300s-timeout behavior after the all-Int marker probe reached layout parse failures; inline spine work removed almost all `--help` heap-spine spills, generalized combinator app reuse cuts self-host `--help` node growth by ~27%, direct known-head/helper dispatch plus C-style `Y` sharing keep the short self-host proxy in the ~60-85 ms band, small-int caching trims final profile nodes to 615,854, strict WHNF coercion fast paths improved the proxy before noisy reruns, F2 numeric/ByteString/WHNF/rnf slices remove several native-recursion paths, the F2 evaluator-owned WHNF spine-loop slice drops the compiler-shaped proxy to Rust `61,450,144` ns/iter vs C `12,341,422`, and the guarded strict-result marker slice improves numeric/bytes common canaries while leaving self-host `--help` noisy/neutral at Rust `67,292,984` ns/iter vs C `12,363,403`; full self-host parity still needs Int comparison/ordering, mixed Int64 shifts/results, helper WHNF frames, and generic strict primops merged into one ordered marker-machine loop |
...
| performance phase | F2 single-stack reducer work active after twenty-nine performance checkpoints plus one benchmark parity checkpoint plus static, dynamic, Rust/LLVM, resolve/shortcut/allocation profile, and `eval.c` lesson checkpoints; numeric and bytes benchmark matrix rows now all sink-match C, the strict numeric/binary-ByteString coercion paths have explicit continuation stacks for Int, Int64 including mixed shifts, Float64, Float32, and binary ByteString marker ops, the normal WHNF driver owns the hot spine/rewrite loop, and guarded strict-result marker forcing for Int/Int64/Float64/Float32/ByteString append has moved into that driver for non-`KnownPrim` heads; rows are split into common-case and rare/specialized; next common-operation targets are one ordered marker stack for helper WHNF frames plus branch-producing comparisons/orderings, mixed Int64 shifts/results, generic strict primops, and remaining repeated resolve/node-classification and app allocation/update mechanics | active |
