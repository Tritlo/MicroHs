...
| parity state | benchmark and smoke sinks match; Rust self-host compile produces byte-identical output. Current runtime uses unconditional 8-byte cells. Latest current-source 128M full self-host gate after the rev-5 profile cfg-gate is 3x byte-identical at `54.648s`, `54.757s`, `54.733s` (median `54.733s`, avg `54.713s`), `3,659,074,945` steps, `31` GCs, median GC pause `8.738s`, high-water `138,068,154` cells, output SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`, and `cell_size_bytes=8`. The later R1 dirty-card work was feature-only, rejected, and reverted; default source is now in the mid-54s band. Most recent fairer-memory 80Mi interval median is `68.975s` with high-water `88.15M` 8-byte cells and 49 GCs. Fresh C oracle full self-host is `47.85s`; older historical gates remain recorded below |
...
| full self-host compile | latest current-source 128M full self-host gate after the rev-5 profile cfg-gate completed byte-identically in three candidate samples: `54.648s`, `54.757s`, `54.733s` (median `54.733s`, avg `54.713s`), `3,659,074,945` steps, `31` GCs, GC pauses `8.738s`, `8.738s`, `8.794s` (median `8.738s`, avg `8.757s`), high-water `138,068,154` cells, sink `661902`, `cell_size_bytes=8`, `cmp=0`, and SHA `29b8c5a55e0952bd25a9a06d5723033e0367ebaf53cfd598fa00f8e65a80d98a`. Paired baseline from the previous committed source was `57.850s`, `57.426s`, `57.380s` (median `57.426s`). Fresh C oracle full self-host completes in `47.85s` with the same output SHA; tagged checkpoint remains `self-hosting-binary-match` at `673.8s`; use `timeout 900s` for full gates and profiling while structural/codegen probes can still regress above the smaller window |
