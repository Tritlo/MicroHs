...
| gate | status |
|---|---|
| Rust fmt/test/release bench build | latest current-source gates after deleting the dead persistent/non-stack reducer layer: `cargo fmt --check`, `cargo check --all-targets`, `cargo check --features profile`, `cargo check --features gc-phase-profile`, `cargo test --lib` 41/41, release `mhs-rust-bench` build, broad source grep for stale persistent/non-stack frame symbols, 100M/32M bounded self-host sanity, 3x 128M full self-host, SHA/cmp, and `git diff --check` passed. Prior broad gates after deleting the rejected `moving-gc` scaffold and stale `eval-phase-profile` layer also passed `cargo check --features profile,gc-phase-profile`, `cargo check --all-targets --features profile`, release `mhs-rust-bench` profile build, release `wasm32-unknown-unknown` build, release `wasm32-wasip1 --bin mhs-rust` build, and profile-enabled 1M smoke. Historical feature-gated moving-GC/eval-phase rows below remain as rejected/retired experiment records only |
| stack rewrite profiling verification | detailed stack phase/shape profiling was retired with the `eval-phase-profile` deletion. Basic `--profile` still works and now prints aggregate head/reduction/app/stack/fallback counters only; the latest profile-enabled 1M smoke completed with `profile_steps=1,001,388`, sink `661902`, and no retired phase/shape sections |
