...
| next moving prototype | replace the conservative full old-space scan with a much cheaper old-to-young edge source. Given S4's per-write barrier-check tax, prefer a coarse card/line dirtying scheme or allocation-site-specific old-update hooks over a branch on every app write; keep the nursery suffix/evacuation machinery as the correctness base |
