...
| full self-host compile | current unconditional packed-cell cleanup was byte-identical in a paired 3x full self-host A/B at the 128M window: committed packed-cell baseline samples `63.229s`, `62.162s`, `62.260s` (median `62.260s`, avg `62.550s`) versus cleanup samples `62.625s`, `62.480s`, `62.135s` (median `62.480s`, avg `62.413s`), same `3,659,075,230` steps, `31` GCs, high-water `138,068,242` cells, sink `661902`, `cell_size_bytes=8`, and `cmp_result=0`. Treat this as performance-neutral cleanup, not a speed claim. Prior accepted packed-cell source win was 16-byte fallback median `63.580s` versus packed median `62.750s`; prior best 128M no-env median remains `61.963s` from the interval probe and should be quoted with heap/RSS annotation. Fresh C oracle full self-host completes in `47.85s` with the same output SHA; tagged checkpoint remains `self-hosting-binary-match` at `673.8s`; use `timeout 900s` for full gates and profiling while structural/codegen probes can still regress above the smaller window |
