...
Last fully verified state: uninitialized 16-slot inline spine checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed before inline spine checkpoint commit; 27 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed before inline spine checkpoint commit |
| `cargo check -p microhs-runtime --bins --quiet` | passed before inline spine checkpoint commit |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed before inline spine checkpoint commit |
...
| `cargo fmt --all --check` | passed before inline spine checkpoint commit |
| `git diff --check` | passed before inline spine checkpoint commit |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release --bin mhs-rust-bench --quiet` | passed before inline spine checkpoint commit |
| `cargo build --release --bin mhs-rust --quiet` | passed before inline spine checkpoint commit |
...
| `arith-chain:200` | 60,353 | 128,243 | 0.47 | yes |
...
| `zoo-chain:300` | 196,186 | 150,913 | 1.30 | yes |
| `data-chain:300` | 205,122 | 153,758 | 1.33 | yes |
...
| `--help` main smoke | 10,810,951 ns/iter; sink `661902` | 123,441,471 ns/iter; sink `1202435` | both run and print usage; not a full compile; refreshed with `--warmup-iters 1 --iters 3` |
...
| measure | count | implication |
|---|---:|---|
| profiled iteration time | 148.701 ms | instrumentation overhead is separate from normal timing |
...
| self-hosting parity | C runtime can execute the compiler comb and produce a new compiler comb | Rust main-mode harness exists and lazy file IO now matches the C CPP scan smoke; latest full-compile run at `34f017fb` still exceeded the old 1,200s cutoff; future bounded attempts use 600s; inline spine work removed almost all `--help` heap-spine spills, but full self-host parity still needs combinator/reducer throughput work |
