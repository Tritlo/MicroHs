...
| local commits ahead after this snapshot commit | 112 |
...
Last fully verified state: F2 unified ordered marker-machine checkpoint.

| gate | status |
|---|---|
| `cargo test -p microhs-runtime --quiet` | passed after F2 unified ordered marker-machine slice; 34 tests |
| `cargo check -p microhs-runtime --lib --quiet` | passed after F2 unified ordered marker-machine slice |
| `cargo check -p microhs-runtime --bins --quiet` | passed after F2 unified ordered marker-machine slice |
| `cargo check --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 unified ordered marker-machine slice |
| `cargo build --target wasm32-unknown-unknown -p microhs-runtime --lib --quiet` | passed after F2 unified ordered marker-machine slice |
...
| `cargo fmt --all --check` | passed after F2 unified ordered marker-machine slice |
| `git diff --check` | passed after F2 unified ordered marker-machine slice |
| `make bin/mhsbench` | passed/up to date at `70eabf4d` |
| `cargo build --release -p microhs-runtime --bins --quiet` | passed after F2 unified ordered marker-machine slice |
...
| self-host Rust main smoke | not yet passing; no-shim run no longer falsely invokes `cpphs`; latest completed full compile run hit `timeout 300s` with exit `124`, printed only the missing `mhs.conf` warning to `/tmp/mhs-selfhost-rust-300s-20260703-041331.out`, left empty stderr, and produced no output comb; detached 1200s launch attempts did not survive the command runner, so a live 900s session is running as session `52834` with output target `/tmp/mhs-selfhost-rust-900s-session-20260703-0429.out.comb` |
...
Latest tracked runtime behavior change is the current F2 unified ordered marker-machine slice: `reduce_whnf_from` now keeps WHNF frames and strict-result markers on one ordered `EvalFrameStack`, so helper-frame subloops can share the same marker path as the normal WHNF driver. Earlier F2 slices remain in place: the evaluator-owned WHNF spine loop, `eval_int`, `eval_int64` including mixed Int64 shifts, `eval_float64`, `eval_float32`, binary ByteString marker ops, the strict WHNF arms `seq`, `IO.strict`, and `isInt`, and `rnf` have explicit stack/continuation paths instead of unbounded native recursion for nested chains. Latest benchmark harness change makes Rust main-mode sinks C-compatible by using the input-byte sink instead of graph-size-dependent output.
...
| `--help` main smoke | 14,499,028 ns/iter; sink `661902` per iter | 61,440,718 ns/iter; sink `661902` per iter | both run and print identical usage text; not a full compile; refreshed with `--warmup-iters 1 --iters 3 --c-mhsbench-mode main` after the F2 unified ordered marker-machine slice; the proxy remains about 4.2x C on this busy machine |
| self-host compiler smoke | 61,920,977,046 ns/iter; output `/tmp/mhs-selfhost-c-current-source-check.comb` is a 647 KiB comb | latest completed Rust run hit `timeout 300s` with exit `124`; `/tmp/mhs-selfhost-rust-300s-20260703-041331.out` only has the missing `mhs.conf` warning, stderr is empty, and no output comb existed; a live 900s foreground session is running as `52834` with output target `/tmp/mhs-selfhost-rust-900s-session-20260703-0429.out.comb` | not at parity; current full-compile check is still pending while the short proxy is sink-comparable |
...
| measure | count | implication |
|---|---:|---|
| profiled iteration time | 100.488 ms | instrumentation overhead is separate from normal timing; raw time is noisy |
...
| profiled resolve calls | 5,028,666 | resolve/node-classification traffic is still much larger than reduction count, but the selected loop removes part of the normal-run overhead around it |
| followed indirections | 99,398 | only about 2.0% of profiled resolve calls follow an indirection |
...
| self-hosting parity | C runtime can execute the compiler comb and produce a new compiler comb | Rust main-mode harness exists and lazy file IO now matches the C CPP scan smoke; latest full C comparison succeeds in `61,920,977,046` ns/iter with sink `661902`; the latest completed Rust full compile still times out at 300s, and a live 900s Rust session is running; inline spine work removed almost all `--help` heap-spine spills, generalized combinator app reuse cuts self-host `--help` node growth by ~27%, direct known-head/helper dispatch plus C-style `Y` sharing keep the short self-host proxy in the ~60-85 ms band, small-int caching trims final profile nodes to 615,854, strict WHNF coercion fast paths improved the proxy before noisy reruns, F2 numeric/ByteString/WHNF/rnf slices remove several native-recursion paths, the F2 evaluator-owned WHNF spine-loop slice drops the compiler-shaped proxy to Rust `61,450,144` ns/iter vs C `12,341,422`, and the unified ordered marker-machine slice keeps self-host `--help` in that band at Rust `61,440,718` ns/iter vs C `14,499,028` while moving branch/mixed strict families into the driver; full self-host parity still needs remaining generic strict primops and per-reduction loop mechanics closer to C |
