...
| next moving prototype | a cheaper old-to-young source must avoid touching every hot app rewrite. Given S4, the rejected exact remembered-source probe, rejected coarse dirty cards, and rejected branchless dirty cards, do not retry hot setter barriers/card marking as standalone R1 work. Prefer allocation-site-specific old-update hooks that stay out of common app rewrites, periodic old-space compaction, or a nursery design that does not need per-rewrite old-to-young tracking; keep the nursery suffix/evacuation machinery as the correctness base |
| avoid | do not optimize the conservative old-space scan as if it were the final collector. It proves root/fixup correctness but becomes O(old) and loses the full gate even at the 128M speed window |
...
| 2 | Continue R1 from the proven `moving-gc` collector bridge only with designs that avoid touching every hot rewrite. The nursery suffix/remapper works, but exact remembered sources, coarse dirty cards, and branchless dirty cards all lost to setter cost. The temporary source-shape profile found old→young edges in old App/Indir cells, not cold arrays/MVars/pointers, so cold-path-only tracking is insufficient. Preserve guest pointer integers by keeping `node_pointers` targets marked/remapped and rebuilding `node_pointer_slots`; next plausible R1 work is allocation-site-specific old App/Indir update handling, periodic old-space compaction, or a nursery shape that does not require per-rewrite old-to-young tracking |
