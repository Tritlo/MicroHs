...
| local commits ahead after this snapshot commit | 59 |
...
| self-host Rust main smoke | not yet passing; no-shim run no longer falsely invokes `cpphs`; latest full compile run at `aa24d02a` hit `timeout 600s` with exit `124` and produced no output comb at `/tmp/mhs-selfhost-rust-aa24d02.comb` |
...
| self-host compiler smoke | 56,387,928,187 ns/iter; output `/tmp/mhs-selfhost-c-refresh.comb` is a 647 KiB `v8.4` comb | latest full run at `aa24d02a` hit `timeout 600s` with exit `124`; no output comb at `/tmp/mhs-selfhost-rust-aa24d02.comb` | not at parity |
...
| self-hosting parity | C runtime can execute the compiler comb and produce a new compiler comb | Rust main-mode harness exists and lazy file IO now matches the C CPP scan smoke; latest full-compile run at `aa24d02a` still hit the 600s timeout with no output comb; inline spine work removed almost all `--help` heap-spine spills, and hot combinator app reuse cuts self-host `--help` node growth by ~19%, but full self-host parity still needs more reducer throughput work |
